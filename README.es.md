...
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\horo`; WSL2 instala en `~/.hermes` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
horo              # ¡empieza a chatear!
```
...
```bash
horo              # CLI interactiva — inicia una conversación
horo model        # Elige tu proveedor y modelo LLM
horo tools        # Configura qué herramientas están habilitadas
horo config set   # Establece valores de configuración individuales
horo gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
horo setup        # Ejecuta el asistente de configuración completo
horo claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
horo update       # Actualiza a la última versión
horo doctor       # Diagnostica cualquier problema
```
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `horo portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway).
...
Hermes tiene dos puntos de entrada: inicia la interfaz de terminal con `horo`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `horo`                                      | Ejecuta `horo gateway setup` + `horo gateway start`, luego envía un mensaje al bot |
...
**Durante la configuración inicial:** El asistente de configuración (`horo setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
horo claw migrate              # Migración interactiva (preset completo)
horo claw migrate --dry-run    # Vista previa de qué se migraría
horo claw migrate --preset user-data   # Migrar sin secretos
horo claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
Consulta `horo claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
