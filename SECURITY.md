# Horo Agent Security Policy

This document describes Horo Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/iscodesnippet2/horo-agent/security/advisories/new).
Do not open public issues for security vulnerabilities. **Horo Agent
...
- Environment details (`horo version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Horo Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Horo Agent's documentation
...
Horo Agent supports two OS-level isolation postures. They address
...
Horo Agent supports this in two ways:

- **Horo Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Horo Agent's in-process heuristics
...
Horo Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Horo Agent
itself. Bugs in Horo Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Horo Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Horo Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Horo Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  agent home/profile security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Horo Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Horo Agent. Bugs in Horo
...
- Horo Agent includes supply-chain guards for MCP server
