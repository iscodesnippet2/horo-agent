...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `horo skills browse` (labeled "official") and install it with `horo skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `horo skills install`.
...
- Integrate with `horo memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Hermes venv, wires the `horo` command, stamps the
install method for `horo update`, and clones the full git project into
...
this way, make sure you run the `horo` entrypoint from this venv; running the
...
```bash
# The standard installer already put `horo` on PATH.
horo doctor
horo chat -q "Hello"
```
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `horo setup` or update `~/.hermes/.env` locally.
...
- **Test it.** Run `horo --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the horo wrapper (it has a stdlib fallback
...
2. **Test manually**: Run `horo` and exercise the code path you changed
...
- Include: OS, Python version, Hermes version (`horo version`), full error traceback
