...
The ``horo update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``pythonw``, not ``horo.exe`` — so the ``taskkill /IM horo.exe`` guard
    ...
    # The horo.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM horo.exe' in text
