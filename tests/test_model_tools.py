...
class TestLegacyToolsetMap:
    def test_expected_legacy_names(self):
        ...
        assert "cronjob_tools" not in _LEGACY_TOOLSET_MAP
...
class TestDisabledToolsetsPlatformBundle:
    ...
    def test_removed_messaging_bundle_resolves_empty(self):
        """Removed messaging bundles must not reintroduce platform tools."""
        from toolsets import bundle_non_core_tools

        assert bundle_non_core_tools("hermes-yuanbao") == set()
        assert bundle_non_core_tools("hermes-discord") == set()
