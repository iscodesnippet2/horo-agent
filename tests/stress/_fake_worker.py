...
exactly the way `horo chat -q` would be, minus the LLM cost.
"""
...
def main():
    ...
    subprocess.run(
        ["horo", "kanban", "heartbeat", tid, "--note", "started"],
    ...
    for i in range(3):
        time.sleep(0.3)
        subprocess.run(
            ["horo", "kanban", "heartbeat", tid, "--note", f"progress {i+1}/3"],
            check=True, capture_output=True,
        )
