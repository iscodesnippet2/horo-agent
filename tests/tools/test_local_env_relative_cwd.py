...
def test_relative_initial_cwd_resolves_from_parent(tmp_path, monkeypatch):
    project = tmp_path / "horo-agent"
    project.mkdir()
    monkeypatch.chdir(tmp_path)

    assert _resolve_local_initial_cwd("horo-agent") == str(project)
...
def test_local_environment_does_not_cd_into_nested_matching_relative_cwd(tmp_path, monkeypatch):
    ...
    env = LocalEnvironment(cwd="horo-agent", timeout=5)
    try:
    ...
    assert "cd: horo-agent" not in result["output"]
