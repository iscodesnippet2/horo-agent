...
class TestRmtreeWritableScopeGuard:
    ...
    def test_refuses_hermes_home_itself(self, tmp_path):
        ...
        hermes.mkdir()
        (horo / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", horo / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(horo)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        skills = horo / "skills"
        skills.mkdir()
        not_skills = horo / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "horo skills reset new-skill" in captured
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `horo profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.hermes) rely on so bundled
    skills are never seeded at install time NOR re-injected by `horo update`.
    """
...
class TestOptOutToggleAndRemove:
    """`horo skills opt-out/opt-in` core: marker toggle + safe removal."""
