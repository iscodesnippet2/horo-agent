...
class TestSecurityGating:
    ...
    def test_policy_is_always_disabled(self, monkeypatch):
        """Environment and config cannot re-enable runtime package installation."""
        monkeypatch.delenv("HERMES_DISABLE_LAZY_INSTALLS", raising=False)
        assert ld._allow_lazy_installs() is False

    def test_legacy_opt_in_cannot_enable(self, monkeypatch):
        monkeypatch.setenv("HERMES_LAZY_INSTALL_TARGET", "/tmp/lazy-packages")
        assert ld._allow_lazy_installs() is False
...
# ---------------------------------------------------------------------------
# active_features + refresh_active_features (Piece A — horo update wiring)
# ---------------------------------------------------------------------------
...
class TestRefreshActiveFeatures:
    ...
    def test_windows_matrix_refresh_is_skipped_before_pip(self, monkeypatch):
        ...
        # path. `horo update` must not retry that doomed install every run.
    ...
    def test_install_failure_recorded_not_raised(self, monkeypatch):
        # A failed refresh must NOT raise out of horo update.
        monkeypatch.setattr(ld, "active_features", lambda: ["test.feat"])
        monkeypatch.setitem(ld.LAZY_DEPS, "test.feat", ("zzzfake==2.0.0",))
        monkeypatch.setattr(ld, "_is_satisfied", lambda spec: False)
        monkeypatch.setattr(ld, "_allow_lazy_installs", lambda: True)
    ...
    def test_lazy_installs_disabled_marked_skipped(self, monkeypatch):
        ...
        # so horo update can render "respecting your config" message.
        monkeypatch.setattr(ld, "active_features", lambda: ["test.feat"])
        monkeypatch.setitem(ld.LAZY_DEPS, "test.feat", ("zzzfake==2.0.0",))
        monkeypatch.setattr(ld, "_is_satisfied", lambda spec: False)
        monkeypatch.setattr(ld, "_allow_lazy_installs", lambda: False)
