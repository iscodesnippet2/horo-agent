...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``horo tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `horo tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``horo tools``,
...
class TestXAIProviderOAuthPath:
    ...
    credential pool (``horo auth`` sign-in) rather than an env-var key.
