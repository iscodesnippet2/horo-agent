...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across horo processes.
...
def test_run_command_tags_hermes_agent_label(monkeypatch):
    """Every container horo-agent starts must carry the hermes-agent=1 label
...
def test_reuse_off_rejects_non_off_egress_container(monkeypatch):
    ...
    (e.g. from before ``horo egress disable``) must be rejected and a
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    ...
    assert "label=hermes-agent=1" in flat, (
        f"horo-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
