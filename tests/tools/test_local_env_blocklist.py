...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        TestHermesBinDirOnPath) so a real ``horo`` on the test runner's PATH
...
class TestHermesBinDirOnPath:
    """The horo install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``horo`` via the terminal tool must work
    even when the gateway was launched without the horo install dir on
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/hermes/bin/hermes" if name == "horo" else None)
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "horo").write_text("#!/bin/sh\n")
    ...
    def test_make_run_env_injects_hermes_bin_dir(self, monkeypatch):
        """A gateway env missing the horo dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._HERMES_BIN_DIR = "/opt/hermes/bin"
