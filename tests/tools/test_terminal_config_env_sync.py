...
at startup, by THREE separate code paths:
  ...
  3. hermes_cli/config.py:set_config_value
                       -> bridges via the canonical ``TERMINAL_CONFIG_ENV_MAP``
                          (one-shot when the user runs ``horo config set …``)
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``horo config set foo bar``.
...
def test_save_config_set_supports_critical_bridged_keys():
    """``horo config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`horo config set terminal.X` doesn't sync these load-bearing "
...
def test_docker_extra_args_is_bridged_everywhere():
    ...
    present in DEFAULT_CONFIG, TERMINAL_CONFIG_ENV_MAP (so ``horo config
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``horo config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``horo config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
