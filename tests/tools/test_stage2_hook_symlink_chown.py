...
def test_stage2_uses_symlink_safe_helper_for_hermes_home_trees(stage2_text: str) -> None:
    ...
    assert 'chown -R hermes:horo "$HERMES_HOME/$sub"' not in stage2_text
    assert 'chown -R hermes:horo "$HERMES_HOME/profiles"' not in stage2_text
    assert 'chown -R hermes:horo "$HERMES_HOME/cron"' not in stage2_text
