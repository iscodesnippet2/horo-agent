...
- Signal forwarding runs through the init so ``docker stop`` triggers
  horo's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, horo accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, horo accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, horo still runs as PID 1 and zombies "
