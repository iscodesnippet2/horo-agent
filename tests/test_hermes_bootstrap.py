...
class TestEntryPointsImportBootstrap:
    ...
    ENTRY_POINTS = [
        "hermes_cli/main.py",   # horo CLI (console_script)
        "run_agent.py",          # horo-agent (console_script)
        "acp_adapter/entry.py",  # horo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``horo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hermes_bootstrap`` as a top-level module) leaves horo
        ...
        """
        # Resolve relative to the horo-agent repo root.  Tests live
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # horo start even when hermes_bootstrap hasn't been
