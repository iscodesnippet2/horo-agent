"""Shared fixtures for the horo-agent test suite.
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # otherwise-unrelated config tests away from the default "horo" host.
    "HERMES_HONCHO_HOST",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HONCHO_HOST", "horo")
...
@pytest.fixture()
def mock_config():
    """Return a minimal horo config dict suitable for unit tests."""
    return {
...
def _live_system_guard(request, monkeypatch):
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `horo update` (or the
        ...
        if "update" in low and (
            # horo update / horo update --gateway / setsid bash -c ... horo update
            ("horo" in low and "update" in low.split())
            or
            ...
            raise RuntimeError(
                ...
                "`horo update` against the real checkout, fetching "
