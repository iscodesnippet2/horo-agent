...
# A representative set of real toolset names. `horo` is deliberately absent —
# that is the corruption #38798 reported (`hermes-cli` rewritten to `horo`).
_KNOWN = {
    "hermes-cli",
    "hermes-telegram",
    "hermes-discord",
...
def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'horo' instead of 'hermes-cli'.
    warnings = validate_platform_toolsets({"cli": ["horo"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'horo'" in w]
...
def test_real_validate_toolset_treats_hermes_cli_valid_and_hermes_invalid():
    ...
    # `hermes-cli` is the real toolset and `horo` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("hermes-cli") is True
    assert validate_toolset("horo") is False
    warnings = validate_platform_toolsets({"cli": ["horo"]}, validate_toolset)
