"""Tests for ``horo dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``horo update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No horo dashboard processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 horo dashboard process(es) running" in out
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``horo dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``horo dashboard --stop`` / ``--status`` actually parse."""
