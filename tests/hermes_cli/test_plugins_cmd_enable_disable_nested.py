...
the *mutation* side: `horo plugins enable/disable` must resolve a bare name
...
class TestEnableDisableNested:
    ...
    def test_enable_clears_manifest_name_alias_from_disabled(
        ...
        ``horo plugins disable web-firecrawl`` gets ``web-firecrawl`` in
...
class TestCompositeMenuWritesCanonicalKey:
    """#40190 follow-up: the interactive `horo plugins` menu must persist
