...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "horo gateway restart",
        "horo gateway stop",
        "horo  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        "kill horo gateway process",
        "pkill -f hermes.*gateway",
        "pkill -f gateway.*horo",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "horo cron list",
        "horo update",
        "horo config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `horo gateway start` is benign — starting a gateway from inside a
        ...
        "horo gateway start",
        "horo gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway horo
        # services must not be falsely blocked (the old `.*horo` matched any
        # horo token).
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hermes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade horo then run horo gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify horo gateway stop/restart refuse when _HERMES_GATEWAY=1."""
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run horo gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("horo gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run horo gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        result = json.loads(cronjob(
            ...
            prompt="please run horo gateway restart nightly",
        ))
