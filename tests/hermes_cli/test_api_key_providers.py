...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_custom_uses_model_api_key_from_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                "model": {
                    "provider": "custom",
                    "base_url": "http://100.98.242.26:1234/v1",
                    "api_key": "cfg-custom-key",
                }
            },
        )

        creds = resolve_api_key_provider_credentials("custom")

        assert creds["provider"] == "custom"
        assert creds["api_key"] == "cfg-custom-key"
        assert creds["source"] == "model.api_key"

    def test_resolve_custom_alias_provider_uses_model_api_key_from_config(self, monkeypatch):
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.delenv("CUSTOM_API_KEY", raising=False)
        monkeypatch.setattr(
            "hermes_cli.config.read_raw_config",
        ...
        assert creds["api_key"] == "cfg-ollama-key"
        assert creds["source"] == "model.api_key"
