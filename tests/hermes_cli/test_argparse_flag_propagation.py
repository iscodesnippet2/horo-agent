...
subparser, placing the flag BEFORE the subcommand (e.g. 'horo --yolo chat')
...
def _build_parser():
    """Build the horo argument parser from the real code.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `horo gateway run --accept-hooks` failed
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `horo <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `horo -t web chat` silently dropped the
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`horo {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `horo --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`horo {flag} chat` lost the flag — got "
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `horo <flag> <value> chat`:\n  "
