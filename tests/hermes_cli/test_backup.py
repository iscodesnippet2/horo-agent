"""Tests for horo backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    ...
    # horo-agent repo (should be EXCLUDED)
    (root / "horo-agent").mkdir(exist_ok=True)
    (root / "horo-agent" / "run_agent.py").write_text("# big file\n")
    (root / "horo-agent" / ".git").mkdir()
    (root / "horo-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestBackup:
    ...
    def test_excludes_hermes_agent(self, tmp_path, monkeypatch):
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "horo-agent" in n]
            assert agent_files == [], f"horo-agent files leaked into backup: {agent_files}"
    ...
    def test_includes_nested_hermes_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        # Add a nested horo-agent directory inside skills (like the real layout)
        nested = hermes_home / "skills" / "autonomous-ai-agents" / "horo-agent"
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root horo-agent must be excluded
            root_agent = [n for n in names if n.startswith("hermes-agent/")]
            assert root_agent == [], f"root horo-agent leaked: {root_agent}"
            # Nested skill horo-agent must be included
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into horo home."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a horo backup."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside horo home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # horo-agent should NOT be present
        assert not (dst_home / "horo-agent").exists()
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without horo markers fails validation."""
        import io
        from hermes_cli.backup import _validate_backup_zip
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        """Backup exits when horo home doesn't exist."""
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        """Backup handles empty horo home (no files to back up)."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside horo root."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text("model: test\n")
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "horo -p coder" in coder_wrapper
...
# ---------------------------------------------------------------------------
# Pre-update backup (horo update safety net)
# ---------------------------------------------------------------------------
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``horo update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``horo backup`` would, and should exclude the same directories."""
        from hermes_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hermes_home=hermes_home)
        assert out is not None
...
# ---------------------------------------------------------------------------
# Pre-migration backup (horo claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``horo claw migrate`` runs before mutating ~/.hermes/."""
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `horo
    ...
    def test_backup_uses_shared_exclusion_rules(self, hermes_home):
        ...
        ``horo backup`` / ``create_pre_update_backup`` — no drift."""
        from hermes_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        ...
        backup — `horo import` should accept it."""
...
class TestRestoreCronJobsIfEmptied:
    """`horo update` config migration can leave cron/jobs.json valid-but-empty,
