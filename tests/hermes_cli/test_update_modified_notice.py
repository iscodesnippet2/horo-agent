"""Guard: every `horo update` path that reports user-modified skills must
also tell the user how to find them.

`horo update` keeps (does not overwrite) bundled skills the user edited and
...
at ``horo skills list-modified`` so the count is actionable — otherwise,
...
_HINT_RE = re.compile(r"horo skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`horo skills list-modified` within the following lines — the "
