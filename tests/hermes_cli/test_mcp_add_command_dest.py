"""Regression test: ``horo mcp add --command`` must not clobber the
...
result, ``horo mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the horo parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`horo mcp add foo --url ...` must keep args.command == "mcp".
