...
When HERMES_HOME is set to the horo root (e.g. systemd hardcodes
...
def _run_apply_profile_override(
    ...
    monkeypatch.setattr(sys, "argv", argv or ["horo", "gateway", "start"])
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    Verifies that HERMES_HOME pointing to the horo root does NOT suppress
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        Bug scenario from #22502: systemd sets HERMES_HOME to the horo root
        and the user switches to a profile via `horo profile use`.
    ...
    def test_hermes_home_already_profile_dir_is_trusted(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["horo", "gateway", "start"])
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        user_home = tmp_path / "home" / "horo"
        profile_dir = user_home / ".hermes" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".hermes").mkdir(parents=True, exist_ok=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SUDO_USER", "horo")
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(os, "geteuid", lambda: 0, raising=False)
        monkeypatch.setattr(sys, "argv", ["horo", "-p", "elias", "gateway", "install", "--system"])
        ...
        assert os.environ.get("HERMES_HOME") == str(profile_dir)
        assert sys.argv == ["horo", "gateway", "install", "--system"]
    ...
    def test_subcommand_profile_flag_is_not_consumed(self, tmp_path, monkeypatch):
        ...
        When that argv is passed through `horo mcp add --args`, the early
    ...
    def test_profile_after_chat_subcommand_is_still_consumed(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            hermes_home=None,
            active_profile="coder",
            argv=["horo", "chat", "-p", "coder", "-q", "hello"],
        )

        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["horo", "chat", "-q", "hello"]
    ...
    def test_top_level_profile_after_value_flag_is_consumed(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            hermes_home=None,
            active_profile="coder",
            argv=["horo", "-m", "gpt-5", "--profile", "coder", "chat"],
        )

        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["horo", "-m", "gpt-5", "chat"]
    ...
    def test_top_level_profile_after_continue_flag_is_consumed(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            hermes_home=None,
            active_profile="coder",
            argv=["horo", "--continue", "--profile", "coder"],
        )

        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["horo", "--continue"]
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``horo gateway run`` (no ``-p``) to mean "the root HERMES_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        gateway is launched as bare ``horo gateway run`` with
        ...
        monkeypatch.setattr(sys, "argv", ["horo", "gateway", "run"])
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `horo gateway run` still honors
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("HERMES_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["horo", "-p", "coder", "gateway", "run"])
