"""Tests for ``horo debug`` CLI command and debug utilities."""
...
class TestRunDebugShare:
    ...
    def test_share_uploads_five_pastes(self, hermes_home, capsys):
        ...
        with patch("hermes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- horo dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``horo debug``
    invocation.
    """
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_survives_sweep_failure(self, hermes_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: horo debug" in out
...
class TestBuildDebugShare:
    ...
    Backs both ``horo debug share`` (CLI) and ``POST /api/ops/debug-share``
...
class TestShareConsentGate:
    """`horo debug share` requires explicit consent before uploading.
