...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `horo update` must still install (the
...
class TestCmdUpdateBranchFlag:
    """``horo update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``horo update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``horo update --branch=<non-main>`` must refuse on the ZIP fallback path.
