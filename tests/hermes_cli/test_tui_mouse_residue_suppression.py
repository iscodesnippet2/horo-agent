...
The Python launcher (`horo --tui …`) has a ~100–300ms cold-start window
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["horo", "--tui", "-c", "abc"])
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `horo --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
