...
between what ``horo model`` shows (``CANONICAL_PROVIDERS``) and what the
...
def test_catalog_covers_every_hermes_model_provider():
    """PARITY CONTRACT: the catalog == the `horo model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `horo model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_hermes_model():
    """The catalog must not invent providers `horo model` doesn't show."""
