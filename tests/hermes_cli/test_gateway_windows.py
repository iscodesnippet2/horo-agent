...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "horo --profile alice gateway start" in out
...
# resume_pending=True) never fires when `horo gateway stop` kills the
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `horo gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
