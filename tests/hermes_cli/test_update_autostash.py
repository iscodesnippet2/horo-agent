...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes horo unrunnable (SyntaxError).
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``horo update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    # Exact flags used by horo update (hermes_cli/main.py).
