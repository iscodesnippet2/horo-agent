...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "horo"))
    ...
    payload = json.loads((tmp_path / "horo" / "auth.json").read_text())
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """horo auth add qwen-oauth must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for horo auth list
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `horo auth add nous` must also populate providers.nous so the
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`horo auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``horo auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``horo auth list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """horo auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``horo auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`horo logout --provider openai-codex` must still clear model.provider.
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `horo logout` should target configured Codex if auth has no active provider."""
    hermes_home = tmp_path / "horo"
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (hermes_home / "config.yaml").write_text(
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `horo auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "horo"))
    hermes_home = tmp_path / "horo"
    hermes_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `horo auth remove`)
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`horo auth remove xai 1` must stick even when the env var is exported
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `horo auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `horo auth remove`.  This is the gate that prevents shell-exported
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `horo auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `horo auth add openai-codex`)
