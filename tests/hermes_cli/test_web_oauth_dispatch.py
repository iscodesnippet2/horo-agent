...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "horo auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "horo auth" not in detail
    finally:
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `horo model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `horo model` but is "
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``horo auth remove
    xai-oauth``.
    """
    from hermes_cli import auth as auth_mod
    from hermes_cli import web_server as ws
    ...
    # Prior `horo auth remove xai-oauth` left the source suppressed.
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "horo auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
