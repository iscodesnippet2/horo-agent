...
base deps) during ``horo update``, leaving the venv in a broken state
...
class TestVerifyCoreDependencies:
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``horo.exe`` on Windows before installing.
        ...
        is left missing and ``horo`` drops off PATH. Previously this path
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `horo update`.
