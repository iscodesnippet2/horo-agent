"""Tests for issue #26670 — concurrent horo.exe detection and improved
quarantine retry / reboot-deferred fallback during `horo update` on Windows.
...
def _make_proc(pid: int, exe: str, name: str = "horo.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "horo.exe").write_bytes(b"")
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "horo.exe")]
...
def test_detect_concurrent_finds_other_hermes_process(_winp, tmp_path):
    ...
    procs = [
        _make_proc(other_pid, str(shim), "horo.exe"),
    ...
    assert result == [(other_pid, "horo.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    upper = str(shim).replace("horo.exe", "HERMES.EXE")
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: horo.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "horo.exe"),
    ]
...
def test_detect_concurrent_still_finds_unrelated_other_hermes(_winp, tmp_path):
    """A sibling horo.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "horo.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent — must be excluded)
    sibling_pid = me + 200  # an UNRELATED horo.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "horo.exe"),
        _make_proc(sibling_pid, str(shim), "horo.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "horo.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "horo.exe"),
        _make_proc(grandparent_pid, str(shim), "horo.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "horo.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    assert result == [(launcher_pid, "horo.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "horo.exe"),
        _make_proc(other_pid, str(shim), "horo.exe"),
    ]
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("horo.exe.old.")
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another horo.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "horo.exe")])
