...
class TestResolveHermesBin:
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'horo' on PATH
    ...
    def test_falls_back_to_path_which(self, monkeypatch):
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/hermes" if name == "horo" else None
        )
...
class TestRelaunch:
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        horo).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_hermes_bin", lambda: r"C:\Users\test\horo.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\horo.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_hermes_bin", lambda: r"C:\horo.exe")
...
class TestResolveHermesBinWindowsPyGuard:
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a horo.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\horo.exe" if name == "horo" else None,
        )

        bin_path = relaunch_mod.resolve_hermes_bin()
        # Must NOT be the .py — must be the horo.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\horo.exe"
    ...
    def test_windows_py_argv0_with_no_hermes_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND horo.exe
