"""Tests for the ``horo send`` CLI subcommand.
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``horo send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``horo config set``) lives
