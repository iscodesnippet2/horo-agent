...
def _install_example_plugin(_isolate_hermes_home):
    ...
    # plugin in exactly as a real operator would with `horo plugins
    # enable example`.
...
class TestWebServerEndpoints:
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "horo"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert fields["workspace"]["placeholder"] == "horo"
        assert fields["aiPeer"]["placeholder"] == "horo"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        assert cfg["hosts"]["horo"]["workspace"] == "myws"
        assert cfg["hosts"]["horo"]["peerName"] == "eri"
        assert cfg["hosts"]["horo"]["environment"] == "local"
        assert cfg["hosts"]["horo"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["horo"]["apiKey"] == "hch-test-key"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_hermes_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("horo", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads((get_hermes_home() / "honcho.json").read_text(encoding="utf-8"))["hosts"]["horo"]
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_hermes_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["horo"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["horo"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(self, monkeypatch, tmp_path):
        ...
        global_path.write_text(
            json.dumps({"baseUrl": "https://kept.example", "hosts": {"horo": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        assert cfg["hosts"]["horo"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_api_key_never_overwrites_oauth_token(self, monkeypatch, tmp_path):
        ...
        path = self._seed_local_honcho({"hosts": {"horo": {"apiKey": "hch-at-oauth-token"}}})
        ...
        assert cfg["hosts"]["horo"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert json.loads(path.read_text(encoding="utf-8"))["hosts"]["horo"]["workspace"] == "myws"
    ...
    def test_list_triggers_config_gated_auto_archive(self):
        ...
        sweeps stale ones (the Desktop `horo serve` code path)."""
        import time as _time
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn horo update")
    ...
    def test_update_hermes_returns_nix_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            raise AssertionError("Nix update guard should not spawn horo update")
    ...
    def test_update_hermes_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn horo update")
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `horo model` but
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `horo import` doesn't auto-abort at the overwrite prompt."""
        import hermes_cli.web_server as ws
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``horo gateway restart`` child (e.g. when a stale
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`horo serve` (HERMES_SERVE_HEADLESS) must NOT serve the SPA even
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``horo model`` custom flow. Regression for the desktop loop where a
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `horo model`), not the first curated paid entry."""
...
class TestBuildSchemaFromConfig:
    ...
    def test_proxy_schema_warns_dashboard_users_about_lifecycle(self):
        ...
        assert "horo egress setup" in entry["description"]
...
class TestNewEndpoints:
    ...
    def test_profile_setup_command_uses_hermes_for_default_profile(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "horo setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        if is_windows:
            assert lines == ["@echo off", "horo -p writer %*"]
        else:
...
# or ``sh -c 'printf …'``) instead of the real ``horo --tui`` binary.  The
