...
`horo gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`horo -p typo gateway start` should print a clear message and
    ...
    assert "horo profile create typo" in out
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `horo gateway run --replace`, the
