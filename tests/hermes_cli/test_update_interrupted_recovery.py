...
``horo update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def test_recovery_self_lock_keeps_core_marker_when_install_fails(
    ...
    # Never clear it solely because horo.exe is an ancestor.
    monkeypatch.setattr(m, "PROJECT_ROOT", tmp_path)
    (tmp_path / "pyproject.toml").write_text("[project]\nname='x'\n")
    m._write_update_incomplete_marker()
...
def test_recovery_self_lock_guard_inactive_when_not_ancestor(tmp_path, monkeypatch):
    # Windows, but horo.exe is NOT in the ancestry (launched via `horo
    ...
    class FakeProc:
        ...
        def parents(self):
            # Ancestry is plain pythons / cmd — no horo.exe shim.
