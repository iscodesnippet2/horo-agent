...
def test_console_parses_bare_and_hermes_prefixed_commands(_isolate_hermes_home):
    ...
    prefixed = engine.execute("horo config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'horo doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'horo setup' to configure\x1b[0m")
        print()
    ...
    assert "horo doctor" not in result.output
    assert "horo setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'horo doctor' for detailed diagnostics"))
        print(osc_link("  Run 'horo setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    ...
    assert "Run `horo skills list`" not in help_text
    assert "Run `horo tools list`" not in help_text
