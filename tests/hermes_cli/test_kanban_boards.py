"""Tests for the multi-board kanban layer (``horo kanban boards …``).
...
* CLI surface: ``horo kanban boards list/create/switch/rm``.
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "horo-agent", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``horo kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
