...
`horo` launches the classic prompt_toolkit REPL by default, but users can
flip ``display.interface: tui`` in config.yaml to make the modern Ink TUI the
default for bare ``horo`` / ``horo chat``. Explicit flags always win:
...
``horo … chat -q`` on a pipe, and the TUI's no-TTY bail-out exits 0
