...
``"horo"`` anywhere (case-insensitive). That false-positived on unrelated
local Modelfiles such as ``hermes-brain:qwen3-14b-ctx16k`` — a tool-capable
Qwen3 wrapper that happens to live under the "horo" tag namespace.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Edge cases
        "",
        "horo",  # bare "horo" isn't the 3/4 family
        "hermes-brain",
