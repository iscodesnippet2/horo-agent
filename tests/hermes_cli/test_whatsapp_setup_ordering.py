...
Before the fix, ``horo whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``horo gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(horo))
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``horo whatsapp`` with an existing paired session and
