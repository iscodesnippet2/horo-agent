"""Tests for the stale-dashboard handling run at the end of ``horo update``.

``horo update`` detects ``horo dashboard`` processes left over from the
...
class TestFindStaleDashboardPids:
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12346, "horo dashboard --port 9120 --no-open"),
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12345, "horo dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_grep_lines_ignored(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(99999, "grep horo dashboard"),
                    _ps_line(12345, "horo dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_filters_specified_pids(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(11111, "horo dashboard --port 9119"),
                    _ps_line(22222, "horo dashboard --port 9120"),
                    _ps_line(33333, "horo dashboard --port 9121"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "horo dashboard --port 9119") + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        must not abort horo update — it's reported as a failure and we
        move on."""
        def fake_kill(pid, sig):
...
class TestWindowsWmicEncoding:
    ...
    `horo update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        ...
        ``None.split('\\n')`` and aborting `horo update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
