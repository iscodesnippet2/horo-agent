...
The notice prints the most recent curator run summary on `horo update`,
...
session start) so users normally never see the rename map. `horo update`
...
def test_prints_multiline_summary_with_rename_map(curator_env):
    ...
    summary = (
        ...
        "full report: horo curator status"
    )
    _set_state(
        curator_env["curator"],
        last_run_at=now,
