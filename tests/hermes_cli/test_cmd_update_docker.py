"""Tests for ``horo update`` / ``--check`` inside the Docker container.
...
fix, ``horo update`` would fall through to ``"✗ Not a git repository.
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``horo update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``horo update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
