...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `horo gateway restart` spawned a
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the horo user. The new
...
# unprivileged horo user from every lifecycle op. These tests run
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "horo -p coder gateway run" in run_text
    assert "s6-setuidgid horo" in run_text
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid horo horo -p coder gateway run --replace" in run_text
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``horo gateway run``, an agent action, the Open WebUI helper) holds
    ...
    # Root profile: bare `horo gateway run --replace` (no -p flag).
    assert "horo gateway run --replace" in default_text
    assert "horo -p default" not in default_text
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "horo profile create typo" in msg
...
#
# `horo gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    dropped horo user and s6-log crash-loops on `mkdir: Permission denied`.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    parent_chown = 'chown hermes:horo "$HERMES_HOME/logs/gateways"'
    ...
    assert 'chown -R hermes:horo "$HERMES_HOME/logs/gateways"' not in log_text
    ...
    leaf_idx = log_text.index('chown -R hermes:horo "$log_dir"')
