"""Tests for hermes_cli.dump._get_git_commit — git SHA resolution for ``horo dump``.

``horo dump`` prints the running commit so support bug reports identify the
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``horo dump`` doesn't have to special-case
