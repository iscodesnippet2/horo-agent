...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running horo containers, those
...
def _make_profile(
    ...
    *,
    ...
    if config:
        ...
        # `horo profile create`. See container_boot._render_run_script.
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    so `horo gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        hermes_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    the root profile. It must call `horo gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    ...
    assert "horo gateway run" in run
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Through s6 /init + the main-wrapper that re-execs `horo`.
        ...
        # Wrapper that kept the explicit `horo` argv0.
        ("/init", "/opt/hermes/docker/main-wrapper.sh", "horo", "dashboard"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/hermes/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/hermes/docker/main-wrapper.sh", "horo", "gateway", "run"),
        ("chat",),
