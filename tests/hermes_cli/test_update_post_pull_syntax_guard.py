"""Tests for the post-pull syntax guard in ``horo update``.
...
becomes unbootable — every ``horo`` invocation imports those files at
...
markers in ``hermes_cli/config.py``; users who ran ``horo update`` in
the 7-minute window before #28458 landed could not run any ``horo``
command afterward.
"""
...
# release; if a future ``horo update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"horo update would brick users. Error: {error}"
    )
