...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        The endpoint must mirror `horo auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `horo auth add`, which clears every suppression for the
...
class TestUpdateCheckEndpoint:
    ...
    ``POST /api/hermes/update`` runs ``horo update``.
    """
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `horo tools` configurator."""
