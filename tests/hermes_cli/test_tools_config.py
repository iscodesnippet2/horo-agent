...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'horo' instead of 'hermes-cli') must warn at resolve time so an
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `horo tools` → X (Twitter) Search to flip the
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `horo tools`,
    ...
    # User explicitly opted into spotify but not x_search via `horo tools`.
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # auto-recovered by _get_platform_tools so saving via `horo tools`
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    ...
    ``horo tools``, but hermes-cli stays in the config and re-enables
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    """Regression: `horo tools` → Configure all platforms → Web Search
...
def test_configure_single_platform_configures_selected_tool_missing_provider(monkeypatch):
    """Regression (per-platform sibling of the global flow): `horo tools` →
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `horo tools` must reach the
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`horo tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'horo-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `horo tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `horo tools`, the toolset appears."""
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `horo tools` → "reconfigure existing" shows
...
#    reported as added/removed by `horo tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `horo tools` printed ``- kanban``
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``horo model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
...
# `horo tools post-setup <key>` child. On Windows each console child (npm,
