...
This saves 500-650ms on ``horo --help``, ``horo version``,
``horo logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``horo --help`` in-process and parse the subcommand block.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["horo", "-w", "--tui"], None),
        (["horo", "version"], "version"),
        (["horo", "--tui", "chat"], "chat"),
        (["horo", "-w", "logs"], "logs"),
        (["horo", "chat", "hello world"], "chat"),
        (["horo", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["horo", "-m", "gpt5", "chat"], "chat"),
        (["horo", "--model", "gpt5", "chat", "hi"], "chat"),
        (["horo", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["horo", "-z", "hello world"], None),
        (["horo", "-z", "hello", "chat"], "chat"),
        (["horo", "--model=gpt5", "chat"], "chat"),     # inline form
        (["horo", "--", "chat"], "chat"),               # -- terminator
        (["horo", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["horo", "some-plugin-cmd"], "some-plugin-cmd"),
        (["horo", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["horo"],                          # bare → chat
        ["horo", "--help"],                # top-level help
        ["horo", "-h"],
        ["horo", "version"],               # known built-in
        ["horo", "logs"],
        ["horo", "gateway", "run"],
        ["horo", "--tui"],
        ["horo", "-w", "--tui"],
        ["horo", "chat", "hi"],
        ["horo", "help"],                  # accepted built-in-ish
        ["horo", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["horo", "meet", "join"],          # potential google_meet plugin
        ["horo", "honcho", "status"],      # potential memory plugin
        ["horo", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``horo help <cmd>`` skips discovery; it won't show
