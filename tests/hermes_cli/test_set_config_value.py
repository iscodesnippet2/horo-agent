...
class TestListNavigation:
    """horo config set must preserve YAML list fields when using numeric
...
class TestSchemaValidation:
    """#34067: ``horo config set`` must not report bare success for
...
class TestDisplaySkinTouch:
    ...
    (`horo config set display.skin X` while it is already X — the recovery
