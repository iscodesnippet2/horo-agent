...
# spawned with `horo -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Worker's perspective (profile activated by `horo -p coder`).
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'horo kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
#
# Workers spawned by the dispatcher must use a `horo` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["horo", ...]`
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    """When `horo` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
...
def test_resolve_hermes_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERMES_BIN", ".\\horo.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_hermes_argv() == [os.path.abspath(".\\horo.exe")]
...
def test_resolve_hermes_argv_hermes_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERMES_BIN", "horo")
...
def test_resolve_hermes_argv_hermes_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    ...
    (tmp_path / "horo.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    Pins the correct module name (NOT `horo` — there is no top-level
    `horo` package). Regression for #23198: the original PR shipped
    `python -m horo` which fails with `No module named horo` on every
    invocation.
    """
    import shutil
    import sys
