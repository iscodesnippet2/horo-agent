...
prompt. Only ``horo --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``horo config set approvals.mode off`` actually bypassed.
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``horo --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
