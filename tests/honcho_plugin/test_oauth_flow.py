    ...
    def do_POST(self):  # noqa: N802
        ...
        if grant_type == "authorization_code":
            body["config"] = {
                ...
                "hosts": {"horo": {"saveMessages": True, "recallMode": "hybrid"}},
            }
        payload = json.dumps(body).encode()
...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    assert saved["hosts"]["horo"]["apiKey"] == "hch-at-1"
    assert saved["hosts"]["horo"]["oauth"]["refreshToken"] == "hch-rt-1"
    assert saved["hosts"]["horo"]["recallMode"] == "hybrid"
    ...
    token, refreshed = oauth.ensure_fresh_token(
        config_path, "horo", now=saved["hosts"]["horo"]["oauth"]["expiresAt"] + 10
    )
    assert refreshed is True
    assert token == "hch-at-2"
    rotated = json.loads(config_path.read_text())["hosts"]["horo"]["oauth"]
...
def test_state_mismatch_is_rejected(fake_as, tmp_path):
    ...
    with pytest.raises(ValueError, match="unknown or expired"):
        oauth_flow.complete_authorization(
            ...
            config_path=tmp_path / "honcho.json", host="horo",
        )
...
def test_client_id_defaults_to_hermes_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "horo-agent"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=horo-agent so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["horo"]["oauth"]["clientId"] == "horo-agent"
...
def test_cli_flow_stores_tokens_without_applying_config(tmp_path, fake_as):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"horo": {"saveMessages": False}}}))
...
def test_device_flow_browser_open_is_caller_opt_in(tmp_path, fake_as):
    ...
    oauth_flow.authorize_via_device_code(
        config_path=config_path, host="horo",
    ...
    config_path.write_text(json.dumps({"hosts": {}}))
    cred = oauth_flow.authorize_via_device_code(
        config_path=config_path, host="horo", sleep=lambda s: None,
    )
...
def test_launcher_runs_flow_in_background_and_reports_connected(monkeypatch, reset_flow):
    ...
    st = oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="horo")
...
def test_launcher_reports_error_on_flow_failure(monkeypatch, reset_flow):
    ...
    oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="horo")
...
def test_launcher_is_idempotent_while_pending(monkeypatch, reset_flow):
    ...
    s1 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="horo")
    assert _wait_until(lambda: len(calls) == 1)  # first flow is running
    s2 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="horo")
    block.set()
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "horo")
    monkeypatch.delenv("HONCHO_API_KEY", raising=False)

    cfgfile.write_text(json.dumps({"hosts": {"horo": {}}}))
    assert oauth_flow.get_flow_status()["connected"] is False

    cfgfile.write_text(json.dumps({"hosts": {"horo": {"apiKey": "hch-v3-static"}}}))
