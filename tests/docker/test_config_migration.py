...
is migrated by docker_config_migrate.py on boot, running as the horo
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    docker_config_migrate.py, running as the horo user."""
    # Start container
    ...
    # Verify config.yaml is owned by horo (migration ran as horo)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "horo", (
        f"config.yaml not owned by horo (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
