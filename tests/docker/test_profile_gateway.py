...
Phase 4 wires `horo -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"horo profile create {PROFILE}")
    ...
    r = _sh(container_name, f"horo -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"horo -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
...
def test_profile_delete_stops_gateway(
    ...
    _sh(container_name, f"horo profile create {PROFILE}")
    _sh(container_name, f"horo -p {PROFILE} gateway start", timeout=60)
    _wait_for_want_state(container_name, want_up=True)

    r = _sh(
        container_name,
        f"horo profile delete {PROFILE} --yes",
        timeout=30,
    )
