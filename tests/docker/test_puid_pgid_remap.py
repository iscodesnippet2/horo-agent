...
Build the real image and verify the actual runtime behavior:
  ...
  1. PUID/PGID env vars remap the horo user UID/GID at boot
...
def test_puid_pgid_remaps_hermes_user(
    ...
    """PUID=1000 PGID=1000 must remap the horo user to UID 1000."""
    ...
    assert r.stdout.strip() == "1000", (
        f"expected horo UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected horo GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
...
def test_hermes_uid_gid_take_precedence_over_aliases(
    ...
    r = docker_exec_sh(container_name, "id -u horo", timeout=10)
    assert r.stdout.strip() == "2000", (
        f"expected horo UID 2000 (HERMES_UID wins), got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g horo", timeout=10)
    assert r.stdout.strip() == "2001", (
        f"expected horo GID 2001 (HERMES_GID wins), got: {r.stdout.strip()}"
    )
...
def test_nas_low_uid_accepted(
    ...
    assert r.stdout.strip() == "99", (
        f"expected horo UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g horo", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected horo GID 100, got: {r.stdout.strip()}"
    )
...
def test_remap_enables_data_volume_writes(
    ...
    """After remap, the horo user must be able to write to /opt/data."""
    start_container(built_image, container_name, "PUID=1000", "PGID=1000")
    ...
    assert "WRITE_OK" in r.stdout, (
        f"horo user cannot write to /opt/data after remap: {r.stdout}"
    )