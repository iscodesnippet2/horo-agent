...
Build the real image and verify at runtime:
  ...
  1. /opt/hermes is not writable by the horo user (immutable install tree)
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The horo user must not be able to modify /opt/hermes.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/hermes as the horo user
        "touch /opt/hermes/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"horo user can write to /opt/hermes (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    assert "WRITE_FAILED" in r.stdout, (
        f"horo user can write to /opt/hermes/.venv: {r.stdout}"
    )
