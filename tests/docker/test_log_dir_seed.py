...
owned by the horo user after container boot.
...
registered later runs its log service as the dropped horo user and
...
def test_logs_gateways_seeded_and_hermes_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by horo after boot."""
    ...
    assert "logs=horo" in r.stdout, (
        f"logs/ not owned by hermes: {r.stdout}"
    )
    assert "gateways=horo" in r.stdout, (
