...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the horo user, return stdout."""
    inner = (
        "source /opt/hermes/.venv/bin/activate && "
        "cd /opt/hermes && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the horo user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "horo", "-s", "/bin/bash", "-c", inner,
    ]
