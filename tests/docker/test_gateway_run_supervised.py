...
Every ``docker exec`` runs as ``horo`` per the conftest module
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → horo gateway run) can take
    ...
    # (main-wrapper.sh → python → horo gateway run) can take well over
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``horo gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``horo gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``horo gateway
    ...
    # (main-wrapper.sh → python → horo gateway run) can take well
    ...
    # started either — so we'd see the CMD's `horo gateway run`
    ...
    # `horo gateway run`. If the recursion guard fails, s6 would
    ...
    assert n <= 1, (
        f"expected at most one supervised python `horo gateway run` "
