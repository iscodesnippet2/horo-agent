...
`docker exec <c> horo login` would write /opt/data/auth.json as
...
1. ``docker exec <c> horo …`` (defaulting to root) gets dropped to the
   horo user before the real binary runs.
2. ``docker exec --user horo <c> horo …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $HERMES_HOME from a ``docker exec`` session land
   as hermes:horo — the actual user-visible invariant.
...
def _wait_for_cont_init(container: str) -> None:
    ...
    finished seeding + ``chown hermes:horo`` config.yaml and running the
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `horo` with a Python-style `-c` shim equivalent — there's no
    ...
    fact that the venv's `horo` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `horo`
    do anything that writes to disk, then check the file's owner.

    Use `horo config set` which writes config.yaml under HERMES_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "horo", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "hermes:horo", (
...
def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user horo already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "horo", sleep_container,
         "horo", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user horo failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up hermes:horo — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "horo", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    assert r.stdout.strip() == "hermes:horo", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected hermes:horo"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    invokes `s6-setuidgid horo horo <args>` which resolves `horo`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> horo login`` (root) wrote
    ...
    for it by writing the same file shape via `horo config set`-style
    ...
    Specifically: pretend the operator ran `horo login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the horo UID. We use `horo auth list` since that touches the
    ...
    # `horo auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `horo config set` first to
    # provoke a write into HERMES_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "horo", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the horo user "
