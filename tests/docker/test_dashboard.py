...
def test_dashboard_not_running_by_default(
    ...
    r = docker_exec(container_name, "pgrep", "-f", "horo dashboard")
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `horo doctor` and any other health check).
...
def test_dashboard_opt_in_starts(
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'horo dashboard'", deadline_s=30.0,
    )
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "horo dashboard",
        )
    ...
    # horo user can kill it (same UID).
