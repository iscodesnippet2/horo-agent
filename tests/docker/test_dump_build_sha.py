"""Regression test: ``horo dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``horo dump`` used to report
...
is absent and ``horo dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/hermes/.hermes_build_sha`` exists in the image, assert that
  ``horo dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's horo user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"horo dump exited {r.returncode}: "
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    ...
    # CI path: build-arg was set, baked file exists.  ``horo dump``
