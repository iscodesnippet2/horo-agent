...
Every ``docker exec`` here runs as the unprivileged ``horo`` user
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = docker_exec(container, "horo", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = docker_exec(container, "horo", "-p", "coder", "gateway", "start", timeout=60)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "horo", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "horo", "profile", "create", "ghost").check_returncode()
...
def test_live_gateway_autostarts_after_real_restart_without_manual_state_stamp(
    ...
    docker_exec(container, "horo", "profile", "create", "live").check_returncode()
    r = docker_exec(container, "horo", "-p", "live", "gateway", "start", timeout=60)
    assert r.returncode == 0, f"gateway start failed: {r.stderr}"
    ...
    # operator `horo gateway stop`), so the shutdown is signal-initiated.
    _docker("restart", container, timeout=60).check_returncode()
