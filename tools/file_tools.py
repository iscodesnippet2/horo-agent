...
def _normalize_without_host_deref(path: str | Path | PurePosixPath) -> PurePosixPath:
    ...
    Remote backends use paths that are meaningful on the target host. Calling
    ``Path.resolve()`` locally can dereference a host-side symlink and rewrite
    the path before the terminal backend sees it.
    """
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return a container-side Hermes mirror prefix.

    Enterprise-lite does not ship container terminal backends, so no mirror
    prefix is active. The helper remains for compatibility with file-safety
    call sites.
    """
    return None
...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    environment yet, creates one using the configured enterprise-lite backend
    (local or ssh) rather than always defaulting to local.
    ...
    with task_lock:
        ...
        if terminal_env is None:
            ...
            # Enterprise-lite keeps local and SSH terminal backends only, so
            # the old container cwd guard is inactive. Keep the branch for
            # compatibility with callers that still import _CONTAINER_BACKENDS.
