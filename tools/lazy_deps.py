...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    # ─── Tools ─────────────────────────────────────────────────────────────
    # Enterprise-lite default tools. These packages are base dependencies in
    # pyproject.toml so air-gapped installs can resolve them from the approved
    # internal wheelhouse up front. The LAZY_DEPS entries stay as compatibility
    # metadata for existing ensure()/feature_specs() call sites; because
    # _allow_lazy_installs() returns False in this build, missing packages fail
    # closed instead of triggering runtime network installs.
    # ACP adapter (VS Code / Zed / JetBrains integration)
    "tool.acp": ("agent-client-protocol==0.9.0",),
    # Dashboard (`horo dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
    ...
    # to the cua-driver process over stdio. The driver binary itself still
    # must be preinstalled by the enterprise image or internal browser/desktop
    # bundle; this entry only describes the Python package requirements.
    "tool.computer_use": (
        "mcp==1.26.0",
...
def _allow_lazy_installs() -> bool:
    """Runtime package installation is forbidden in the enterprise-lite build.

    All optional dependencies must be resolved from the operator's approved
    internal wheelhouse before the process starts.  Preserve the function and
    its callers so missing optional features continue to fail closed without
    changing the proven agent loop.
    """
    return False
...
def _pkg_name_from_spec(spec: str) -> str:
    ...
    ``"example-pkg>=1.18.0,<2"`` → ``"example-pkg"``
    ``"example[extra]>=0.20"`` → ``"example"``
    """
...
def _specifier_from_spec(spec: str) -> str:
    ...
    ``"example-pkg==2.2.0"`` → ``"==2.2.0"``
    ``"example[extra]>=0.20,<1"`` → ``">=0.20,<1"``
...
def ensure_and_bind(
    ...
    *,
    ...
    Example usage: call ``ensure_and_bind("feature.name", importer, globals())``
    from a feature module that is optional in the lite build.
    """
    try:
        ensure(feature, prompt=prompt)
