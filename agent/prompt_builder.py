...
# than on the machine where Hermes itself runs. Enterprise-lite keeps SSH as
# the only remote terminal backend.
_REMOTE_TERMINAL_BACKENDS = frozenset({"ssh"})
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    try:
        ...
        # Build the environment the same way tools/terminal_tool.py does for
        # a live SSH command. Enterprise-lite has no container/cloud terminal
        # backends, so image/container config stays empty.
        image = ""
...
def _current_session_platform_hint() -> str:
    ...
    session_context = sys.modules.get("agent.session_context")
