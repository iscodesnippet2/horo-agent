...
# display_hermes_home imported lazily at call sites (stale-module safety during horo update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  horo config set model.provider custom")
    print_info("  horo config set model.base_url http://localhost:8080/v1")
    print_info("  horo config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'horo setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'horo setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'horo setup' to configure"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'horo setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('horo setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('horo setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('horo setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('horo setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('horo setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('horo config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('horo config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('horo config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('horo', Colors.GREEN)}              Start chatting")
    print(f"   {color('horo gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('horo doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `horo tools` and `horo setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``horo model``)
    ...
    provider added to ``horo model`` is automatically available here.
    ...
    # Delegate to the shared horo model flow — handles provider picker,
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``horo model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via horo setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'horo setup tts')."""
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `horo setup agent` later to customize.")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  horo config edit")
    print_info("   Open config in your editor:  horo config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'horo setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     horo config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: horo gateway install")
            else:
                print_info("  You can install later: horo gateway install")
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: horo gateway install --system")
                print_info("  Or run in foreground:  horo gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   horo gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   horo gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `horo setup tools` and `horo tools` use the same flow:
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: horo claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: horo claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use horo claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      horo setup           — full or quick (auto-detected)
      horo setup model     — just model/provider
      horo setup tts       — just text-to-speech
      horo setup terminal  — just terminal backend
      horo setup gateway   — just messaging platforms
      horo setup tools     — just tool configuration
      horo setup agent     — just agent settings
    """
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'horo setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'horo config edit'")
    ...
    # Tune later with `horo setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'horo setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    horo setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  horo setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``horo tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``horo setup agent`` /
    ``horo config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `horo update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        horo tools")
        print_info("  Seed skills:         horo skills opt-in --sync")
        print_info("  Add MCP servers:     horo mcp add")
        print_info("  Enable plugins:      horo plugins")
        print_info("  Tune agent settings: horo setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`horo update` runs from re-injecting them. Opt back in any")
            print_info("time with `horo skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `horo tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `horo plugins list` / `horo plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `horo plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `horo mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `horo mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   horo tools")
    print_info("  Seed skills:         horo skills opt-in --sync")
    print_info("  Add MCP servers:     horo mcp add")
    print_info("  Tune agent settings: horo setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'horo setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'horo setup gateway'.")
