...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``horo --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            "Re-sync version files (e.g. run 'horo update', or set "
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `horo profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `horo doctor --fix`, or "
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        ...
        check_warn("Gateway service linger", f"(could not check: {e})")
        return
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `horo`.
    os.environ.setdefault("HERMES_INTERACTIVE", "1")

    # Handle `horo doctor --ack <id>` as a fast path. Persist the ack and
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`horo doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            ...
            check_warn(
                ...
                "(run `horo update`; fixed versions: 3.51.3+ / 3.50.7 / "
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'horo setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'horo setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'horo setup' to create one")
                issues.append("Run 'horo setup' to create .env")
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'horo config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'horo setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'horo setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'horo config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'horo setup' to migrate config")
                else:
                    issues.append("Run 'horo doctor --fix' or 'horo setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'horo doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `horo setup` run)",
                )
                if should_fix:
        ...
        try:
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'horo doctor --fix' "
                        "(or 'horo sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'horo doctor --fix' "
                        "(or 'horo sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'horo doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "horo"
        ...
        _cmd_link = _cmd_link_dir / "horo"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(horo not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `horo update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `horo doctor` invocation.
    ...
    _section("Configured Model Endpoint")
    try:
        ...
        api_key = str(model_cfg.get("api_key") or "").strip() or get_env_value("OPENAI_API_KEY")
        api_mode = str(model_cfg.get("api_mode") or "chat_completions").strip()

        if provider and provider != "custom":
            check_warn("Configured provider is not enterprise-lite default", f"({provider})")
            issues.append("Set model.provider to custom for the enterprise-lite build")
        else:
            check_ok("Provider", "(custom OpenAI-compatible)")
        if model_name:
            check_ok("Model configured", f"({model_name})")
        else:
            _fail_and_issue("Model not configured", "", "Set model.default in config.yaml", issues)
        if base_url:
            check_ok("Base URL configured", f"({base_url})")
        else:
            _fail_and_issue("Base URL not configured", "", "Set model.base_url in config.yaml", issues)
        if api_key:
            check_ok("API key configured")
        else:
            check_warn("API key not configured", f"(set OPENAI_API_KEY in {_DHH}/.env)")
        if api_mode == "chat_completions":
            check_ok("API mode", "(chat_completions)")
        else:
            check_warn("API mode", f"({api_mode}; enterprise-lite expects chat_completions)")
    except Exception as e:
        check_warn("Configured model endpoint", f"(could not check: {e})")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'horo setup' to configure missing API keys for full tool access")
    except Exception as e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "horo -p" in content:
                            _m = _re.search(r"horo -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'horo doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
