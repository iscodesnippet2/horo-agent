"""CLI handlers for the ``horo proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "horo proxy requires aiohttp. Run `horo setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"horo auth add {adapter.name}")
        print(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print(
        "\nStart the proxy with: horo proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``horo proxy <subcommand>``."""
    ...
    print(
        "horo proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  horo proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  horo proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  horo proxy providers\n"
