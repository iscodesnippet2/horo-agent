...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.horo-agent/skills/ for newly installed or removed skills",
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``horo skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``horo skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``horo skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("horo", "Talk to Hermes or run a subcommand", "[subcommand] [args]"))
    seen.add("horo")
