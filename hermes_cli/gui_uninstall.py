...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``horo desktop`` / ``horo gui``)
...
``horo uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``horo desktop`` inside the checkout.
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove horo' or "
            "'sudo dnf remove horo'). AppImage builds are a single file you "
