...
def _resolve_api_key_provider_secret(
    ...
    from hermes_cli.config import get_env_value_prefer_dotenv, read_raw_config
    ...
    # Enterprise-lite/custom endpoint contract: allow config.yaml model.api_key
    # to satisfy the generic custom provider when model.provider resolves to the
    # same local/custom family. This keeps CLI, wheel, and WebUI consistent for
    # self-hosted OpenAI-compatible endpoints instead of requiring an extra env
    # var export on top of config.yaml.
    if provider_id == "custom":
        try:
            ...
            if isinstance(model, dict):
                ...
                provider_aliases = {
                    "custom", "local", "ollama", "vllm", "llamacpp", "llama.cpp", "llama-cpp"
                }
                if model_provider in provider_aliases:
                    for key_name in ("api_key", "api"):
                        cfg_val = str(model.get(key_name) or "").strip()
                        if has_usable_secret(cfg_val):
                            return cfg_val, f"model.{key_name}"
        except Exception:
            pass
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``horo auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `horo model` to re-authenticate."
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``horo auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``horo auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``horo auth add <provider>`` inside the profile, profile entries
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `horo logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'horo doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'horo model' for available providers, or run 'horo doctor' to diagnose config issues."
        raise AuthError(msg, code="invalid_provider")

    # Explicit one-off CLI creds always mean the generic custom provider.
    ...
    raise AuthError(
        "No inference provider configured. Run 'horo model' to choose a "
        "provider and model, or set an API key (for example OPENAI_API_KEY) in ~/.hermes/.env.",
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: horo auth add nous",
        provider="nous",
        code=reason,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `horo model` walks users into a broken Qwen setup flow.
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `horo auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `horo auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `horo auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `horo auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print("       App name:     anything (e.g. horo-agent)")
    ...
    # Persist so subsequent `horo auth spotify` runs skip the wizard.
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `horo auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `horo auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `horo auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `horo auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``horo setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``horo auth add openai-codex``
      ...
      ``horo auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    with _auth_store_lock():
        ...
        # ``horo auth add openai-codex`` created (which must not be
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    if response.status_code == 429:
        ...
        # "run horo auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `horo auth` to re-authenticate."
            )
            relogin_required = True
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `horo model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `horo model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `horo model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `horo model`.",
            provider="xai-oauth",
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``horo auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `horo model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `horo model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # with a clear error so the user can re-run `horo model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``horo model``
...
# so a new `horo --profile <name> auth add nous --type oauth` can one-tap
...
# ``%LOCALAPPDATA%\horo`` on native Windows, or the Docker/custom root.
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "refresh endpoint. For health checks, use `horo auth status` "
            "instead.\n"
            "Re-authenticate with: horo auth add nous"
        )
        relogin = True
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``horo auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``horo auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `horo auth add nous --type oauth`. Best-
...
# ~350ms even on the failure path, and read-only UI surfaces (`horo tools`, status panels,
# subscription-feature checks) call it many times per render — `horo tools` → "All Platforms"
...
# `horo auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `horo auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `horo auth` and
    # `horo model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``horo doctor`` runs the live
        ...
        try:
            ...
            if not installed:
                ...
                info["hint"] = (
                    ...
                    "is skipped here. Run `horo doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `horo logout` historically keyed off auth.json.active_provider only.
...
def login_command(args) -> None:
    """Deprecated: use 'horo model' or 'horo setup' instead."""
    print("The 'horo login' command has been removed.")
    print("Use 'horo auth' to manage credentials,")
    print("'horo model' to select a provider, or 'horo setup' for full setup.")
    raise SystemExit(0)
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``horo auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``horo auth list`` would show
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `horo model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``horo status`` that just want to know
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `horo model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `horo model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Run `horo model` or configure an API key to use Hermes.")
        else:
