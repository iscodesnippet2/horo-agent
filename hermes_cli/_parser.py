"""
Top-level argparse construction for the horo CLI.
...
_EPILOGUE = """
Examples:
    horo                        Start interactive chat
    horo chat -q "Hello"        Single query mode
    horo --tui                  Launch the modern TUI (or set display.interface: tui)
    horo --cli                  Force the classic REPL (overrides display.interface: tui)
    horo -c                     Resume the most recent session
    horo -c "my project"        Resume a session by name (latest in lineage)
    horo --resume <session_id>  Resume a specific session by ID
    horo setup                  Run setup wizard
    horo logout                 Clear stored authentication
    horo auth add <provider>    Add a pooled credential
    horo auth list              List pooled credentials
    horo auth remove <p> <t>    Remove pooled credential by index, id, or label
    horo auth reset <provider>  Clear exhaustion status for a provider
    horo model                  Select default model
    horo fallback [list]        Show fallback provider chain
    horo fallback add           Add a fallback provider (same picker as `horo model`)
    horo fallback remove        Remove a fallback provider from the chain
    horo config                 View configuration
    horo config edit            Edit config in $EDITOR
    horo config set model gpt-4 Set a config value
    horo gateway                Run messaging gateway
    horo -s hermes-agent-dev,github-auth
    horo -w                     Start in isolated git worktree
    horo gateway install        Install gateway background service
    horo sessions list          List past sessions
    horo sessions browse        Interactive session picker
    horo sessions rename ID T   Rename/title a session
    horo logs                   View agent.log (last 50 lines)
    horo logs -f                Follow agent.log in real time
    horo logs errors            View errors.log
    horo logs --since 1h        Lines from the last hour
    horo debug share             Upload debug report for support
    horo console                Open the safe Hermes command console
    horo update                 Update to latest version
    horo dashboard              Start web UI dashboard (port 9119)
    horo dashboard --stop       Stop running dashboard processes
    horo dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    # Mirrors `horo chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "under model.provider — use `horo setup` or edit the file to change it."
        ),
    )
    parser.add_argument(
        "-t",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.horo-agent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    # top-level parser: when the user writes `horo -m foo chat`, argparse
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.horo-agent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
