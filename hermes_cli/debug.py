"""``horo debug`` debug tools for Hermes Agent.

Currently supports:
    horo debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[horo debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``horo debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``horo debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `horo debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``horo debug share`` invocation added ~20 MB of resident Python
    ...
    ``horo debug share`` also runs an opportunistic sweep as a fallback
...
def _capture_dump() -> str:
    """Run ``horo dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``horo dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``horo debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "horo debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `horo debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  horo debug delete <url>")
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``horo debug share --nous``: upload the bundle to Nous-S3.
    ...
    if redact:
        logger.info(
            "horo debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `horo debug share --local` to print the report instead, "
            "or `horo debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: horo debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'horo debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``horo debug`` call.
    ...
    # best-effort — any failure is swallowed so ``horo debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: horo debug <command>")
        print()
        print("Commands:")
