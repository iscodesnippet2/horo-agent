...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: horo --checkpoints")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `horo journey`).
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run horo chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `horo gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `horo sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `horo sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `horo -c`/`--resume`. The startup resume
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hermes_cli.curator so the CLI and the `horo curator`
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``horo bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: horo bundles create "
        ...
        _cprint(
            ...
            f"Manage with `horo bundles`.{_RST}"
        )
    ...
    def _handle_update_command(self) -> bool:
        ...
        ``horo update`` so the user sees update output directly and gets
        ...
        raw = self._prompt_text_input_modal(
            ...
            detail="This will exit the current session and run `horo update`.",
            choices=choices,
        )
        if raw is None:
