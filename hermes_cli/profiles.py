...
Usage::
    ...
    horo profile create coder          # fresh profile + bundled skills
    horo profile create coder --clone  # also copy config, .env, SOUL.md, skills
    horo profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    horo -p coder chat                 # or via flag
    horo profile use coder             # set as sticky default
    horo profile delete coder          # remove profile + alias + service
"""
...
#
# Rationale per item:
#   horo-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `horo profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `horo
...
# `horo skills install` or drop SKILL.md files into the profile's skills/.
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "horo-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "horo", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the horo root, NOT to the current HERMES_HOME
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``horo``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``horo`` profile inside ``~/.hermes/``) or get refused
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, horo subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a horo subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hermes_exe = shutil.which("horo") or "horo"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `horo doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``horo -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``horo profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``horo -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``horo profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``horo update`` skips re-seeding this profile's
    ...
    # Keys pages, `horo -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `horo update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`horo profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `horo update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `horo profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `horo -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``horo profile create
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    # Never terminate ourselves or a parent (e.g. `horo -p <canon> profile
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: horo profile create {canon}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: horo profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any horo modules
