"""CLI subcommand: `horo curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `horo curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`horo curator status` and run `horo curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`horo curator unpin {args.skill}`"
        )
        return 1
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`horo curator backup` or wait for the next curator run."
            )
        else:
            print(
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="horo curator")
    register_cli(parser)
