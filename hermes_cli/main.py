...
Usage:
    horo                     # Interactive chat (default)
    horo chat                # Interactive chat
    horo gateway             # Run gateway in foreground
    horo gateway start       # Start gateway as service
    horo gateway stop        # Stop gateway service
    horo gateway status      # Show gateway status
    horo gateway install     # Install gateway service
    horo gateway uninstall   # Uninstall gateway service
    horo setup               # Interactive setup wizard
    horo logout              # Clear stored authentication
    horo status              # Show status of all components
    horo cron                # Manage cron jobs
    horo cron list           # List cron jobs
    horo cron status         # Check if cron scheduler is running
    horo doctor              # Check configuration and dependencies
    horo honcho setup                    # Configure Honcho AI memory integration
    horo honcho status                   # Show Honcho config and connection status
    horo honcho sessions                 # List directory → session name mappings
    horo honcho map <name>               # Map current directory to a session name
    horo honcho peer                     # Show peer names and dialectic settings
    horo honcho peer --user NAME         # Set user peer name
    horo honcho peer --ai NAME           # Set AI peer name
    horo honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    horo honcho mode                     # Show current memory mode
    horo honcho mode [hybrid|honcho|local]  # Set memory mode
    horo honcho tokens                   # Show token budget settings
    horo honcho tokens --context N       # Set session.context() token cap
    horo honcho tokens --dialectic N     # Set dialectic result char cap
    horo honcho identity                 # Show AI peer identity representation
    horo honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    horo honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    horo version             Show version
    horo update              Update to latest version
    horo uninstall           Uninstall Hermes Agent
    horo acp                 Run as an ACP server for editor integration
    horo sessions browse     Interactive session picker with search

    horo claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``horo update``
...
# this guard, horo crashes on import and the user can't run
# ``horo update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
# a prior ``horo update`` left a recovery marker and a core package's import
...
def _set_process_title() -> None:
    """Set the process title to 'horo' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``horo tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``horo.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("horo")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"horo", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"horo")
        # Windows: the .exe name is already ``horo.exe`` — nothing to do.
    except Exception:
        pass
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = str(get_default_hermes_root() / "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    cron jobs, pipes run ``horo … chat -q`` with stdio on a pipe. This
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`horo --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``horo --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (horo tools, horo setup, horo model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'horo {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any horo module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.horo-agent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `horo mcp add ... --args <command argv>`.
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo horo -p <name>` against the invoking user's home.
        ...
        candidate = home / ".horo-agent" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`horo chat -p coder`), so keep scanning broadly.
    ...
    # parent directory name (e.g. ~/.horo-agent/profiles/coder or
    # /opt/data/profiles/coder).  If HERMES_HOME points to the horo root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `horo profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # 2. If no flag, check active_profile in the horo root.
    #
    ...
    # reserved ``gateway-default`` slot runs bare ``horo gateway run`` to mean
    ...
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent horo from starting
            print(
...
# Initialize centralized file logging early — all `horo` subcommands
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'horo')
    """
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo horo {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo horo {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  horo --tui --resume {target}")
    if title:
        print(f'  horo --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@horo" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("HERMES_HOME") or str(get_default_hermes_root())
    try:
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``horo update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        ...
        "This usually means `horo update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hermes checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `horo --tui`\n"
        "If the checkout is still inconsistent, run `horo update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `horo --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `horo update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``horo kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``horo kanban boards switch`` from another session can flip the file
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``horo … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'horo sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'horo doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'horo setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `horo gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: horo gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `horo gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  horo gateway")
        ...
        print()
        print("  Or install as a service: horo gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'horo whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``horo whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``horo model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'horo model' for "
                "available providers, or run 'horo doctor' to diagnose config "
                "issues."
            )
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``horo model``
...
# `horo model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `horo model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``horo model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
# the catalog eagerly costs ~55ms on every `horo` invocation — including
# fast paths like `horo --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``horo setup`` / ``horo model``.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `horo setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_slack(args):
    ...
    Dispatches ``horo slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: horo slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `horo slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``horo`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``horo update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``horo update``
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``horo`` launcher present but ``hermes_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the horo venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — horo venv python3, no nix-shell overhead
    ...
    # a horo venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``horo update`` to stash the
    lockfile — repeatedly.
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``horo web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (horo web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``horo update`` can unconditionally call ``horo desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``horo desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``horo update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "linux-unpacked" / "horo",
            release_dir / "linux-unpacked" / "Hermes",
            release_dir / "linux-arm64-unpacked" / "horo",
...
# `horo update` → `horo desktop --build-only` → relaunch) rebuilds
...
def _ensure_desktop_exe_launchable(
    ...
    if restored is not None:
        ...
        print("    Your existing version was kept and still works. Run `horo desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `horo desktop --force-build` to rebuild, or re-run the Hermes")
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``horo desktop --build-only`` → electron-builder ``--dir``).
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `HERMES_DESKTOP_DISABLE_GPU=... horo desktop` keeps working.
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  horo gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # horo update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `horo update` path).
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> horo desktop --force-build")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``horo dashboard`` processes other than ourselves.

    ``horo dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``horo update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``horo serve`` as
    ...
    patterns = [
        ...
        # The headless backend (`horo serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "horo.*dashboard"`) keeps us consistent
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `horo update`.
    ...
    print("  Preview now:  horo curator run --dry-run")
    print("  Pause it:     horo curator pause")
    print(
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `horo update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `horo sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    horo sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  horo sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``horo update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``horo update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: horo curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``horo dashboard`` processes.

    Called at the end of ``horo update`` (default ``reason``) and also
    from ``horo dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``horo update`` path), a detected
    ...
    if killed:
        ...
        print("    horo dashboard --port <port>")
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `horo update "
            f"--branch {branch}`, or update against main with `horo update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for horo-agent itself
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: horo skills list-modified  "
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes horo completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `horo update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``horo update``, every profile is now current.
    """
    homes = []
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``horo update``.
    ...
    protocol stream (``horo acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `horo update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``horo.exe`` launch always has the launcher as an
    ...
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from horo.exe; applying package-only first aid, "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``horo update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    names = set(_load_console_script_names()) or {"horo", "horo-agent", "horo-acp"}
...
def _detect_concurrent_hermes_instances(
    ...
    The Hermes Desktop Electron app spawns ``horo.EXE`` as a backend child,
    so during ``horo update`` the user-invoked process and the desktop's
    ...
    shims (``horo.exe`` / ``hermes-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``horo update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``horo.exe``) is a separate process from the Python
    ...
    # setuptools-generated horo.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``horo update``
    ...
    #      horo.exe sitting *under* a non-Hermes parent (e.g. a Hermes
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "horo.exe"
    lines = ["✗ Another horo.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Hermes Desktop, exit any open `horo` REPLs, and")
    lines.append("  stop the gateway (`horo gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `horo update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``horo.exe``.
    ...
    when ``horo update`` runs, ``horo.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``horo.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next horo invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hermes Desktop, exit other `horo` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `horo update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``horo.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``horo.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``horo`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``horo.exe.old.*`` left by prior updates.

    Called early on every horo invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``horo.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``horo.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `horo update`:")
    print(
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``horo update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `horo update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `horo update` once the upstream issue is resolved.")
        return True
    if status == "indeterminate":
        print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``horo.exe`` / ``hermes-gateway.exe`` shims
    ...
    # hours later inside ``horo update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``horo.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``horo.exe`` shim is locked during ``horo update``, or when uv/distlib
    skips a launcher write. The symptom is ``horo-agent.exe`` and
    ``horo-acp.exe`` present but ``horo.exe`` missing, so ``horo`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `horo update --force` after "
            "closing other horo processes."
        )
        return
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``horo update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``horo.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``horo`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `horo update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `horo update --force` after closing other horo processes."
        )
        return
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `horo update` is exactly the step
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `horo update`.")
            failed = ["repo root"]
            if any(
    ...
    # need desktop during `horo update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `horo update`.")
        return list(labels)
    ...
    # hung. The chatty npm-deprecation noise during `horo update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``horo update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``horo update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``horo update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``horo update --gateway``) the update is already
...
def _log_only_write(text: str) -> None:
    ...
    During ``horo update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``horo update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``horo update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``horo update`` without requiring a reinstall.  The
    ...
    ``horo`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/hermes.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v horo'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of HERMES_HOME under
      ``backups/`` (restorable via ``horo import``; the #48200 wrong-path
    ...
    print(f"  Restore:  horo import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``horo update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``horo update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run horo from any interpreter), so report healthy to
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The horo.exe shim guard misses the biggest lock-holder class on
    ...
    calling process and its ancestors are always excluded (a CLI ``horo
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines.append("    horo update")
    lines.append("  (or use `horo update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    horo.exe concurrent-instance guard does not see them. They still import
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: horo gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``horo gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``horo update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `horo update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another horo.exe is holding the venv shim
    ...
    # (most commonly the Desktop app's `horo serve` backend) will keep .pyd
    ...
    # the horo.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Hermes windows/gateways and re-run: horo update")
            else:
                print("✓ Already up to date!")
        ...
        # every user who ran ``horo update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``horo
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``horo update`` again later once a fix lands.")
                    else:
        ...
        # marker survives and the next ``horo`` launch finishes the install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `horo` again "
        ...
        # build.  ``horo desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``horo desktop`` shouldn't be forced into a full
        # Electron build by ``horo update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            ...
            # when `horo update` runs inside the desktop updater chain
            # (Desktop → hermes-setup → horo update), the shell PATH
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT, env=_build_env)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `horo desktop` to retry)")
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'horo config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: horo config migrate")
            else:
                print()
                print("Skipped. Run 'horo config migrate' later to configure.")
        else:
        ...
        # check `horo curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``horo update`` gives users a
        ...
        # When running as ``horo update --gateway`` (spawned by the gateway's
        ...
        try:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: horo gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `horo update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: horo gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: horo -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both horo.service (from a pre-rename install) and the
        # current hermes-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `horo update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("  These pre-rename units (horo.service) fight the current")
                print("  hermes-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    horo gateway migrate-legacy")
                print()
        ...
        print("  horo model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``horo -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `horo profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → horo -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  horo profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     horo -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `horo profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → horo -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  horo -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      horo -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `horo profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"horo -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
...
def _report_dashboard_status() -> int:
    """Print ``horo dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``horo dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No horo dashboard processes running.")
        return 0

    print(f"{len(pids)} horo dashboard process(es) running:")
    for pid in pids:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    ``.../app.asar.unpacked/dist``. A standalone ``horo dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No horo dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `horo update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with horo serve")
    ...
    # those vars then runs `horo dashboard` would otherwise:
...
# so that in ``horo -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``horo -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``horo`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `horo --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for horo CLI."""
    # Cosmetic: make the process show up as 'horo' instead of 'python3.11'
    ...
    # Sweep stale ``horo.exe.old.*`` quarantine files left by previous
    # ``horo update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``horo update``
    ...
    # ``horo skills install update``) merely defers recovery one launch;
    # under-matching (missing ``horo -p work update``) would race a recovery
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `horo model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.horo-agent/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo horo uses to snapshot working directories before "
    ...
    # subcommand — ``horo --help``, ``horo version``, ``horo logs``,
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `horo computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `horo tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `horo computer-use doctor` to run cua-driver's\n"
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: horo computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: horo computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: horo computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: horo computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: horo computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: horo computer-use permissions grant")
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <horo home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # `horo sessions prune`: no time window and no filters at all
            ...
            # Launch horo --resume <id> by replacing the current process
            ...
            if result.get("vacuumed") is False:
                print("  (VACUUM was skipped or failed — run "
                      "`horo sessions optimize` later to reclaim freed space.)")
    ...
    # run `horo desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``horo -c Pokemon Agent Dev`` → ``horo -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'horo -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (horo hooks list, cron
    ...
    # ``horo egress start`` refusing when credential_source=bitwarden
