"""MCP picker — interactive `horo mcp picker` (also the default `horo mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``horo mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `horo plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `horo mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: horo mcp install <name>    Picker: horo mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `horo update` "
...
def show_catalog() -> None:
    """`horo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`horo mcp picker` (and default `horo mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`horo mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `horo mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
