...
Config files are stored in ~/.horo-agent/ by default:
- ~/.horo-agent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.horo-agent/.env         - API keys and secrets

This module provides:
- horo config          - Show current configuration
- horo config edit     - Open config in editor
- horo config get      - Print a resolved configuration value
- horo config set      - Set a specific value
- horo config unset    - Remove a user configuration value
- horo config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``horo config set`` (which rewrites
    ...
    ``config.yaml`` in place: horo never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.horo-agent/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``horo logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  horo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``horo update``.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    ``docker`` and ``horo update`` refuses to run ("doesn't apply inside the
    ...
    #    container, and honouring it wrongly blocks ``horo update``.
    try:
        method = (
...
# Long-form text for ``horo update`` / ``--check`` when running inside the
...
✗ ``horo update`` doesn't apply inside the Docker container.
...
pulling a fresh image and restarting your container instead:
  ...
  docker compose up -d --force-recreate horo-agent
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``horo update`` inside Docker.
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_name = info.get("container_name", "horo-agent")
    exec_user = info.get("exec_user", "horo")
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the horo group can
...
def ensure_hermes_home():
    ...
    if key in _HERMES_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``horo profile create``).
...
DEFAULT_CONFIG = {
    ...
    # Opt in via ``horo chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: horo sweeps the checkpoint base at startup
        ...
        # ``horo checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # to compact its own thread (default); horo =
    ...
    # kanban dashboard (see ``horo dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``horo kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``horo kanban decompose`` and the kanban
        ...
        # ``horo profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `horo model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `horo` (and `horo chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `horo --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `horo -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `horo --tui` drops a one-time hint
        ...
        # CLI, TUI, and desktop app. Manage with `horo pets`. Disabled until
    ...
    # cron daemon.
    #
    # See `horo curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `horo curator run --consolidate` overrides
        ...
        # normally restored on every `horo update`, so pruning them only
        ...
        # user can roll back with `horo curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `horo doctor --ack
    ...
    # todos to ready, and fires `horo -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `horo` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `horo kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     horo-agent's own python (sys.executable). Maximum isolation
    ...
    "sessions": {
        ...
        # default of ``horo sessions prune``.
        "retention_days": 90,
        ...
        # user runs `horo sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `horo update` prints a one-line notice with
    ...
    "updates": {
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``horo backup``-style zip
        #     of HERMES_HOME into <HERMES_HOME>/backups/, restorable with
        #     ``horo import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `horo update` does with uncommitted local changes to the
        ...
        # Refresh an already-installed cua-driver during `horo update`.
...
OPTIONAL_ENV_VARS = {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'horo-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `horo config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `horo update`.  Skill-config prompting is a
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'horo setup' to create a valid config")]
    ...
    # scalars are bridged into os.environ (gateway/run.py, horo send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'horo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.horo-agent")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "horo update / horo -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `horo plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `horo curator status` has no
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `horo config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `horo config set`"
                    )
    ...
    # behavior set it to true explicitly via `horo config set`.
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: horo config set <key> <value>")
    ...
    # `horo update`, but only the version bump is persisted.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.horo-agent/.env); "
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``horo config set``
    ...
    backends) — while display paths (``horo status``/``dump``) read ``name``
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.horo-agent/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.horo-agent/config.yaml.
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.horo-agent/config.yaml.\n
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.horo-agent/.env.
    ...
    interactive menu render (`horo tools`, `horo setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `horo tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.horo-agent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.horo-agent/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.horo-agent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.horo-agent/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.horo-agent/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.horo-agent/.env`` over ``os.environ``.
...
def show_config():
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'horo doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  horo config edit     # Edit config file", Colors.DIM))
    print(color("  horo config set <key> <value>", Colors.DIM))
    print(color("  horo setup           # Run setup wizard", Colors.DIM))
    print()
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``horo config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``horo config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``horo config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `horo config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `horo config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `horo config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: horo config get <key> [--json]")
            print()
            print("Examples:")
            print("  horo config get model")
            print("  horo config get terminal.backend")
            print("  horo config get skills.config --json")
            sys.exit(1)
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: horo config unset <key>")
            print()
            print("Examples:")
            print("  horo config unset model")
            print("  horo config unset terminal.backend")
            print("  horo config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'horo config migrate' to add them")
        ...
        print("  horo config           Show current configuration")
        print("  horo config edit      Open config in editor")
        print("  horo config get <key>          Print a resolved config value")
        print("  horo config set <key> <value>   Set a config value")
        print("  horo config unset <key>        Remove a config value")
        print("  horo config check     Check for missing/outdated config")
        print("  horo config migrate   Update config with new options")
        print("  horo config path      Show config file path")
        print("  horo config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``horo config`` UI so users
