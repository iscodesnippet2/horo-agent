    ...
    *,
    ...
    if account_info is None:
        return (
            ...
            f"is unavailable. Run `horo model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                ...
                f"`horo model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `horo model`. "
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `horo model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            ...
            f"login, so {capability} is unavailable. Run `horo model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `horo model` to refresh Hermes."
        return message
