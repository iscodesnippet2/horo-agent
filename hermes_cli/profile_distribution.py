...
* ``horo profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``horo skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``horo profile``, not a parallel tree):

    horo profile install <source> [--name N] [--alias] [--force] [--yes]
    horo profile update  <name>  [--force-config] [--yes]
    horo profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "horo-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``horo profile update`` can
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `horo profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`horo profile install <source> --name {canon} --force`."
        )
