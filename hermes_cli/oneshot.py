...
configured for "cli" in `horo tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `horo chat`:
...
from agent.session_context import declare_stateless_channel
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"horo -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "horo -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"horo -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "horo -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "horo -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "horo -z: --provider requires --model (or HERMES_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"horo -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("horo -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``horo -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when horo is invoked for
