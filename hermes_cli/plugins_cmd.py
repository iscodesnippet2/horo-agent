"""``horo plugins`` CLI subcommand — install, update, remove, and list plugins.
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `horo plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `horo plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  horo gateway restart[/dim]")
    console.print()
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``horo setup`` / ``horo plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``horo plugins enable`` /
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`horo plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    so ``horo plugins list`` can show and enable them.
    """
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``horo plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] horo plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] horo plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] horo plugins")
    console.print("[dim]Enable/disable:[/dim] horo plugins enable/disable <name>")
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``horo plugins enable web/firecrawl`` cleared
...
def plugins_command(args) -> None:
    """Dispatch horo plugins subcommands."""
