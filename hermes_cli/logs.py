"""``horo logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    horo logs                    # last 50 lines of agent.log
    horo logs -f                 # follow agent.log in real time
    horo logs errors             # last 50 lines of errors.log
    horo logs gateway -n 100    # last 100 lines of gateway.log
    horo logs gui -f            # follow gui.log (dashboard/pty/ws)
    horo logs desktop -f        # follow desktop.log (Electron app boot/backend)
    horo logs --level WARNING    # only WARNING+ lines
    horo logs --session abc123   # filter by session ID substring
    horo logs --component tools  # only tool-related lines
    horo logs --since 1h         # lines from the last hour
    horo logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Hermes runs — try 'horo chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'horo chat' to generate logs)")
