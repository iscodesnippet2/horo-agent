...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``horo doctor --ack <id>``; the ack is persisted to
...
1. ``horo doctor`` (and ``horo doctor --ack <id>``)
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including horo when configured "
        ...
        remediation=(
            ...
            "After cleanup: horo doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'horo doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``horo doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `horo doctor` on the gateway host for details.")
