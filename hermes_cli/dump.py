...
def _dotenv_key_names() -> set[str]:
    ...
    exports. ``horo debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _redact(value: str) -> str:
    ...
    ``horo dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `horo auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `horo auth list` shows it (#42130).
