...
try:
    ...
    # running `horo dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``horo gateway run`` — but the
    desktop app spawns a ``horo dashboard`` backend, not a gateway, so a cron
...
async def _lifespan(app: "FastAPI"):
    ...
    # since the app has no gateway running the scheduler. Server `horo
...
class ModelAssignment(BaseModel):
    ...
    # ``horo model`` custom flow collects. Honored only on the main slot for
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``horo update``.
    ...
    the dashboard's ``horo update`` button is the correct update path and
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``horo status`` and the
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the horo home root).
    """
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``horo <subcommand>`` detached and record the Popen handle.
    ...
    # _HERMES_GATEWAY=1. Inheriting it makes a spawned `horo gateway restart`
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["horo", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``horo gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``horo gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``horo gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``horo update`` in the background."""
    ...
    try:
        ...
        _log.exception("Failed to spawn horo update")
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/hermes/update`` actually runs ``horo update``.
...
def get_sessions(
    ...
    try:
        ...
        try:
            ...
            # `horo serve` backend. No-op when disabled or run recently.
...
async def get_egress_status():
    """Enterprise-lite keeps the endpoint but disables the feature."""
    return {"text": "Egress proxy is not included in enterprise-lite."}
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `horo model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``horo model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``horo model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `horo model` provider — would be
        ...
        # Vertex — a `horo model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `horo model` picker uses (not desktop-only prefix guesses).
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in horo gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in horo gateway setup",
...
# the canonical ``horo auth add <provider>`` command so the dashboard
...
# Guarded so horo web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``horo auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_hermes_oauth_file
    oauth_file = _get_hermes_oauth_file()
    payload = {
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``horo auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``horo model`` re-login path in _login_xai_oauth).
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `horo sessions stats`).
...
def _maybe_auto_archive_for_profile(db, profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``horo serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `horo sessions prune`)."""
    has_window = (
...
# servers managed here show up under `horo mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `horo mcp catalog` / `horo mcp install` read.  ``profile`` scopes
...
# spawn the real `horo gateway <verb>` so behaviour matches the CLI exactly.
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `horo auth remove`) can seed again.
        # Mirrors the `horo auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``horo auth remove`` uses: each source cleans up its
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `horo import`. The spawned action runs with
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``horo`` subprocess, and
...
# Human-readable labels for each hub source id (matches `horo skills search`
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`horo -p <name> skills install`)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``horo profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``horo profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``horo profile describe
    <name> --auto``.
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``horo tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `horo tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``horo tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``horo tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``horo tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``horo -p <profile> tools post-setup``.
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``horo computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``horo --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``horo --tui`` would run.  Tests monkeypatch this
    ...
    — the same propagation ``horo -p <name>`` performs. The in-process
...
def mount_spa(application: FastAPI):
    ...
    """
    # `horo serve` is the headless backend: it must NEVER serve the browser
    ...
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (horo serve): web UI disabled — use "
            "`horo dashboard` for the browser UI."
            if _headless
            ...
            try:
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`horo plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
