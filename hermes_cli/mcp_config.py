"""
MCP Server Management CLI — ``horo mcp`` subcommand.

Implements ``horo mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``horo mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  horo mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  horo mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  horo mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: horo mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `horo mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  horo mcp add <name> --url <endpoint>')
        _info('  horo mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``horo mcp login`` and
    ``horo mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `horo mcp remove` + `horo mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `horo mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``horo mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``horo mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: horo mcp reauth <name>   |   horo mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'horo mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``horo mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `horo plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("horo mcp                                    Open the catalog picker (default)")
        _info("horo mcp catalog                            List Nous-approved MCPs")
        _info("horo mcp install <name>                     Install a catalog MCP")
        _info("horo mcp serve                              Run as MCP server")
        _info("horo mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("horo mcp add <name> --command <cmd>         Add a stdio server")
        _info("horo mcp add <name> --preset <preset>       Add from a known preset")
        _info("horo mcp remove <name>                      Remove a server")
        _info("horo mcp list                               List configured servers")
        _info("horo mcp test <name>                        Test connection")
        _info("horo mcp configure <name>                   Toggle tools")
        _info("horo mcp login <name>                       Re-authenticate OAuth")
        _info("horo mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
