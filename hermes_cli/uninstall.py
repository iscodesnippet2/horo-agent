...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing horo-agent or horo PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hermes Agent" comment and following line
                if '# Hermes Agent' in line or '# horo-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('horo' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing horo
                if 'horo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the horo wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "horo",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'horo-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``horo`` command:
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``horo gateway run`` processes
...
#   4. On the ``horo dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # all get swept.  Also match the bare horo-agent install dir.
    markers = [root + "\\horo-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HERMES_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "horo".  We don't want to catch
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``horo -p <name> gateway stop|uninstall`` because
    ...
    #    Use `python -m hermes_cli.main` so we don't depend on a `horo`
...
def run_gui_uninstall(args):
    ...
    Mirrors ``horo uninstall --gui``. Removes the desktop app's built
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Hermes agent at {hermes_home / 'horo-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hermes agent is still installed. Run 'horo' to use the CLI,")
    print("or 'horo uninstall' to remove the agent too.")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``horo`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    ...
    log_info("Removing horo command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.hermes/, just remove the horo-agent subdir
