...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `horo --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``horo auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``horo auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`horo auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `horo auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `horo auth` is called bare."""
