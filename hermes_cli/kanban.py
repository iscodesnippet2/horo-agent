"""CLI for the Hermes Kanban board — ``horo kanban …`` subcommand.
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    Used by ``horo kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`horo kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `horo kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `horo kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `horo project list`.")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `horo gateway start`.",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``horo kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: horo kanban <action> [options]\n"
                "Run 'horo kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `horo kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    with board_scope:
        ...
        # every `horo kanban repair` into "could not initialize database"
...
# ---------------------------------------------------------------------------
# Boards management (horo kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``horo kanban boards <action>``.
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `horo kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `horo kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `horo kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `horo kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("Create one with `horo -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  horo gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `horo -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``horo kanban show <id>``
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `horo kanban complete <id>` from the terminal tool and
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "horo kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    horo gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`horo kanban list --status ready` / "
                    f"`horo kanban list --status blocked` for recent "
