...
This module backs ``horo console`` and is intentionally narrower than the
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'horo doctor'")
        and last.startswith("Run 'horo setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="horo", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `horo "):
        return ""
    return summary
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `horo {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `horo {usage}`?",
        )
...
class HermesConsoleEngine:
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        ...
        if first in blocked_top:
            return f"`horo {first}` is not available in Hermes Console."
        blocked_pairs = {
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from horo console")
...
def run_console_repl(
    *,
    ...
    """Run the local ``horo console`` REPL."""
