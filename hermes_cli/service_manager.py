...
``horo gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``horo gateway start/stop/restart``).
    """
    ...
    # `horo gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged horo
    ...
    container drops to horo via ``s6-setuidgid``, that silent failure
...
# active (set via the ``horo -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `horo profile create`
...
# UID/GID of the in-image ``horo`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the horo user.
    ...
    (the ``horo`` user — UID 10000 — running every Hermes runtime
    ...
    is dispatched as the horo user.
    ...
    with horo ownership before triggering ``s6-svscanctl -a``,
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the horo user already — directory is hermes-
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``horo -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`horo profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the horo user and exec's
             ``horo -p <profile> gateway run`` (or just ``horo
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``horo gateway
        ...
        # (a stray ``horo gateway run`` from a shell, an agent action, or
        ...
        if profile == "default":
            gateway_cmd = "horo gateway run --replace"
        else:
            gateway_cmd = f"horo -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid horo {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``horo logs`` reads and what
        ...
        return (
            ...
            # later then runs its log service as horo and crash-loops on
            ...
            f'chown hermes:horo "$HERMES_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R hermes:horo "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid horo s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``horo gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``horo -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with horo ownership
            ...
            # the horo user) won't hit EACCES on root-owned 0700
