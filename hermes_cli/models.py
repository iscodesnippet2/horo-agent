...
Add, remove, or reorder entries here — both `horo setup` and
`horo` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `horo model`
...
# this list:  horo model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `horo model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `horo model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("lmstudio", "LM Studio", "LM Studio or another internal OpenAI-compatible model server"),
]
...
# slug as a top-level row in the interactive `horo model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``horo model``, the
...
# ``horo model``) uses the richer free/paid-tier-aware resolver — see
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``horo auth add openai-codex`` without
    ``horo model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``horo model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    horo-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — horo-agent
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``horo model``, ``/model``, etc.).
    """
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``horo auth add copilot`` and by ``_seed_from_env``
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``horo model`` shows.
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=horo"
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `horo moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
