"""
horo fallback — manage the fallback provider chain.
...
Subcommands:
  horo fallback [list]   Show the current fallback chain (default when no subcommand)
  horo fallback add      Pick provider + model via the same picker as `horo model`,
                           then append the selection to the chain
  horo fallback remove   Pick an entry to delete from the chain
  horo fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `horo model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  horo fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `horo model`, then append the selection to the chain."""
    ...
    print("  `horo model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `horo fallback list` to view, or `horo fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``horo fallback [subcommand]``."""
