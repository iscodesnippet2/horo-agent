"""Shell completion script generation for horo CLI.
...
complete -F _hermes_completion horo
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef horo
# Hermes Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(horo completion zsh)"
...
{top_cmds_str}
            ...
            _describe 'horo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c horo -f -s p -l profile"
