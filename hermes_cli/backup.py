"""
Backup and import commands for horo CLI.

`horo backup` creates a zip archive of the entire ~/.hermes/ directory
(excluding the horo-agent repo and transient files).

`horo import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
# ``horo-agent`` is special-cased to root level only in ``_should_exclude``
...
_EXCLUDED_DIRS = {
    "horo-agent",     # the codebase repo — re-clone instead
...
# File names that ``horo import`` must never overwrite, matched by basename so
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to horo root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``horo-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/hermes-agent/`` — must be preserved.
        if part == "horo-agent" and part != parts[0]:
            continue
        return True
...
# to the user as a hung `horo update` (#70553 follow-up). Sessions databases
...
def run_backup(args) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``horo-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "horo-agent" and not is_root)
        ]
    ...
    if not errors:
        print(f"\nRestore with: horo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a horo home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a horo dir name
        if prefix in {".hermes", "horo"}:
            return prefix + "/"
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: horo profile list  (after installing horo)")

        # Guidance
        print()
        if not (hermes_root / "horo-agent").is_dir():
            print("Note: The horo-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: horo update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  horo -p {pname} gateway install")

        print("Done. Your Hermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and horo backup --quick)
# ---------------------------------------------------------------------------
...
# — `horo update` snapshots this set before pulling so approved-user lists
...
def create_quick_snapshot(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: horo snapshot list"
        )
        logger.error(
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``horo update``.
...
def run_quick_backup(args) -> None:
    """CLI entry point for horo backup --quick."""
...
def create_pre_update_backup(
    ...
    (``horo update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `horo claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `horo import` and the
