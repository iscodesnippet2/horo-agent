...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'horo model' or "
...
def resolve_runtime_provider(
    *,
    ...
    if requested_provider in ("vertex", "google-vertex", "vertex-ai", "gcp-vertex", "vertexai"):
        ...
        if not token or not base_url:
            raise AuthError(
                ...
                "Run `horo setup` to install Vertex support."
            )
        return {
            "provider": "vertex",
            "api_mode": "chat_completions",
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `horo auth list`). If the key is
