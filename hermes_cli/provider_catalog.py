...
The provider list shown by ``horo model`` (CLI/TUI) and the desktop Settings
...
the same universe ``horo model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``horo model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `horo model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``horo model`` universe.
