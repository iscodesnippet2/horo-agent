...
`horo tools` and `horo setup tools` both enter this module.
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``horo tools post-setup <key>`` child. On Windows, every console child
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``horo tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``horo tools`` checklist actually offers
    for ``platform``.
    ...
    so ``horo tools`` never claims to add or remove a toolset the user was
...
# Enterprise-lite has one runtime surface. Messaging-platform composites are
# removed at source level; the CLI/WebUI share the same core toolset.
PLATFORMS = {
    "cli": {"label": "CLI / WebUI", "default_toolset": "hermes-cli"},
    "webui": {"label": "CLI / WebUI", "default_toolset": "hermes-cli"},
}
...
# provider+model picker like `horo model` — NOT this single-key prompt, so
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``horo update`` and
      by ``horo computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `horo update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `horo computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `horo update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `horo update`,
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in horo-agent directory)")
            return
        ...
        try:
            ...
            _print_info("    Run manually: horo plugins enable observability/langfuse")
...
def run_post_setup_command(args) -> int:
    """``horo tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: horo tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[hermes-cli, spotify]`` after enabling Spotify via ``horo
    ...
    # otherwise saving via `horo tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `horo tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `horo tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by horo tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `horo` instead of
    ...
    # at runtime, not only during the next `horo update`/`horo doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `horo tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `horo tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `horo tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `horo model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``horo model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `horo model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `horo model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `horo tools` and `horo setup tools`.
    ...
    print(color("  Changes take effect on next 'horo' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, horo mcp
