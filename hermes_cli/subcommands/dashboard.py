"""``horo dashboard`` / ``horo serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `horo dashboard` / `horo serve` cmdlines and SIGTERM them directly —
    # the same path `horo update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
...
def build_dashboard_parser(
    ...
    # backend as `horo dashboard --no-open --tui --host ... --port ...`. The
    ...
    # `horo dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.horo-agent/.env.
    # Nested subparser so bare `horo dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.horo-agent/.env, and prints "
            "how to engage the login gate. Requires being logged in (horo setup)."
        ),
    )
