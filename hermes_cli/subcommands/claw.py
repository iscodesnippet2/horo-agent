...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.horo-agent/ (by default a "
        "single restore-point archive is written to ~/.horo-agent/backups/ "
        "before apply; restorable with 'horo import').",
    )
    claw_migrate.add_argument(
