...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hermes Agent. Use 'horo debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    horo debug share              Upload debug report (asks for confirmation)
    horo debug share --yes        Skip confirmation (for scripts/CI)
    horo debug share --lines 500  Include more log lines
    horo debug share --expire 30  Keep paste for 30 days
    horo debug share --local      Print report locally (no upload)
    horo debug share --no-redact  Disable upload-time secret redaction
    horo debug share --nous       Upload to Nous-internal storage (private)
    horo debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'horo debug share'",
    )
