"""horo hooks — inspect and manage shell-script hooks.

Usage::

    horo hooks list
    horo hooks test <event> [--for-tool X] [--payload-file F]
    horo hooks revoke <command>
    horo hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``horo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: horo hooks {list|test|revoke|doctor}")
        print("Run 'horo hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `horo hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `horo hooks test` and `horo hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `horo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `horo hooks doctor` would execute
