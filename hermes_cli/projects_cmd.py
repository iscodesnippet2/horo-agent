"""``horo project`` CLI — manage first-class, multi-folder Projects.
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``horo project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: horo project <action> [options]\n"
                "Run 'horo project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `horo project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `horo project add-folder`.",
            file=sys.stderr,
        )
        return 1
