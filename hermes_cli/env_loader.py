...
# Used by setup / `horo model` flows to label detected credentials so
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `horo setup` (or edit the "
...
def load_hermes_dotenv(
    *,
    ...
    if hermes_home or os.getenv("HERMES_HOME"):
        home_path = Path(hermes_home or os.getenv("HERMES_HOME", ""))
    else:
        from hermes_constants import get_default_hermes_root

        home_path = get_default_hermes_root()
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `horo model`
