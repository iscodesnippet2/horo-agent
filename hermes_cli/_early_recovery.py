...
The ``horo`` console entry point is ``hermes_cli.main:main``.  Importing
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``horo acp`` speaks JSON-RPC on stdout);
...
def recover_if_needed(
    ...
    recovery marker from a prior ``horo update`` exists AND an import probe
