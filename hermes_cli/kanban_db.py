...
``horo -p <profile>`` joins the same board as the dispatcher that
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``horo kanban boards
...
# enough that kebab-case names like ``atm10-server`` or ``horo-agent``
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``horo kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``horo kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``horo kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``horo kanban log`` unambiguous even when multiple
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``horo gateway run --replace`` /
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``horo kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `horo kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``horo kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``horo --skills ...``. Use this to pin a task to a
    ...
    # `horo --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``horo kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `horo kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``horo kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``horo kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``horo kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``horo kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``horo kanban complete <id>``) works without requiring
...
#      via ``horo kanban show <id>`` and the dashboard.
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``horo kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``horo kanban
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``horo -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `horo kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    ...
    # pyproject.toml, NOT a top-level ``horo`` package — there is no
    # ``horo`` package to import.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``horo`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("horo")`` — the console-script shim, normalized to
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Hermes is launched from a venv and the ``horo`` shim is not on
    ...
    hermes_bin = _safe_which_no_cwd("horo") if _IS_WINDOWS else shutil.which("horo")
    if hermes_bin:
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``horo -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `horo -p <name>` the
    ...
    # that even when the worker activates a profile (`horo -p <name>`
    ...
    # `horo -p <assignee>` activates the profile, but the env var is
    ...
    # older horo builds on PATH that predate the flag's precedence.
    ...
    # `horo kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`horo` executable not found on PATH. "
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``horo kanban daemon`` is systemd-friendly.
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``horo kanban assignees`` for the terminal.
