...
process replacement so that ``horo sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``horo`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the horo entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("horo")`` on PATH.
    ...
    ``horo.exe`` on PATH or the ``sys.executable -m hermes_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh horo invocation.
    ...
    relaunch contract wants: "run horo again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``horo.exe`` (a Python console-script shim that wraps
    ...
    That's functionally equivalent — the user sees "horo exited, then
    new horo started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``horo`` not on PATH yet (install
