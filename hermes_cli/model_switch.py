...
# The previous substring check (`"horo" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "horo" in their tag but are fully tool-capable.
#
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``horo config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'horo model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'horo doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Build curated model lists keyed by horo provider ID
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # /model picker sees the SAME list `horo model` would build, with
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `horo model` picker deliberately shows ONLY the curated agentic
