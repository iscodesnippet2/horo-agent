"""`horo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hermes/checkpoints/``.  Actions:

    horo checkpoints               # same as `status`
    horo checkpoints status        # total size, project count, breakdown
    horo checkpoints list          # per-project checkpoint counts + workdir
    horo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    horo checkpoints clear [-f]    # nuke the entire base (asks first)
    horo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    horo checkpoints
    horo checkpoints prune --retention-days 3 --max-size-mb 200
    horo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: horo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``horo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `horo checkpoints` → status
