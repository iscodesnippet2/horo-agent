...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``horo -p <profile> auth``). No HTTP side effect.
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``horo kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`horo kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"horo config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"horo config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: horo kanban specify {task_id}",
            payload={"command": f"horo kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: horo -p {assignee} doctor",
            payload={"command": f"horo -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: horo -p {assignee} auth",
            payload={"command": f"horo -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: horo kanban log {task_id}",
                payload={"command": f"horo kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: horo kanban events {task_id}",
            payload={"command": f"horo kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
    ...
    * Dispatcher is misconfigured (wrong board, wrong HERMES_HOME).
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "horo kanban diagnostics"},
        ),
    ]
