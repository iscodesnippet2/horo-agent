"""Per-provider model-selection wizard flows for ``horo setup`` / ``horo model``.
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # bad paste short of re-running `horo setup` from scratch. OpenRouter
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `horo moa configure <name>` first.")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `horo auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``horo auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `horo model` to set a model.")
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so horo knows where to find it
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'horo model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
