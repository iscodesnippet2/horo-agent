...
def configure_windows_stdio() -> bool:
    ...
    # already-running shells don't see the change — which means horo
...
def _default_windows_editor() -> str:
    ...
    ``horo config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``horo`` from right after install) retain their old PATH.
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "horo", "git", "cmd"),
        os.path.join(local_appdata, "horo", "git", "bin"),
        os.path.join(local_appdata, "horo", "git", "usr", "bin"),
        # Hermes venv Scripts directory — host of the horo.exe shim itself,
        ...
        os.path.join(local_appdata, "horo", "horo-agent", "venv", "Scripts"),
