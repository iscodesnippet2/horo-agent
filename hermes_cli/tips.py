...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or horo -c.",
    ...
    # --- CLI Flags ---
    "horo -c resumes your most recent CLI session. horo -c \"project name\" resumes by title.",
    "horo -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "horo -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "horo chat -t web,terminal enables only specific toolsets for a focused session.",
    "horo chat -s github-pr-workflow preloads a skill at launch.",
    "horo chat -q \"query\" runs a single non-interactive query and exits.",
    "horo chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "horo chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "horo --yolo bypasses all dangerous command approval prompts for the entire session.",
    "horo chat --source telegram tags the session for filtering in horo sessions list.",
    "horo -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "horo doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "horo dump outputs a compact setup summary — great for bug reports.",
    "horo config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "horo config edit opens config.yaml in your default editor.",
    "horo config check scans for missing or stale configuration options.",
    "horo sessions browse opens an interactive session picker with search.",
    "horo sessions stats shows session counts by platform and database size.",
    "horo sessions prune --older-than 30 cleans up old sessions.",
    "horo skills search react --source skills-sh searches the skills.sh public directory.",
    "horo skills check scans installed hub skills for upstream updates.",
    "horo skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "horo skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "horo mcp add github --command npx adds MCP servers from the command line.",
    "horo mcp serve runs Hermes itself as an MCP server for other agents.",
    "horo auth add lets you add multiple API keys for credential pool rotation.",
    "horo completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "horo logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "horo backup creates a zip backup of your entire Hermes home directory.",
    "horo profile create coder creates an isolated profile that becomes its own command.",
    "horo profile create work --clone copies your current config and keys to a new profile.",
    "horo update syncs new bundled skills to ALL profiles automatically.",
    "horo gateway install sets up Hermes as a system service (systemd/launchd).",
    "horo memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "horo webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: horo tools disables unused tools, horo skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "horo models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as horo -w).",
    ...
    "Profile names become shell commands — 'horo profile create coder' creates the 'coder' command.",
    "horo profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "horo sessions export backup.jsonl exports all sessions for backup or analysis.",
    "horo -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "horo skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: horo cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "horo gateway install sets it up as a system service that starts on boot.",
    ...
    "Customize SOUL.md to shape the agent behavior.",
    ...
    # --- MCP ---
    "horo mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "horo mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "horo mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — horo tools can toggle them per platform.",
    ...
    "horo chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "horo plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "horo acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "horo auth add nous or horo auth add openai-codex sets up OAuth-based providers.",
    ...
    "horo status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'horo profile create ls' would be rejected.",
    "horo profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "horo claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via horo tools.",
    ...
    # --- Curator ---
    'horo curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "horo curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'horo curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'horo auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'horo dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'horo dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'horo -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'horo chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'horo chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'horo chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "horo chat --source tool tags programmatic chats so they don't clutter horo sessions list.",
    'horo dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'horo sessions rename <ID> "new title" renames any past session; horo sessions delete <ID> removes one.',
    'horo import restores a full Hermes backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'horo fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'horo pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'horo setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'horo status --deep runs the full health sweep across every component; plain horo status is the quick view.',
    ...
    '--source tool chats are excluded from horo sessions list by default — set --source explicitly to see them.',
