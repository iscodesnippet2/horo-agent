...
def reconcile_profile_gateways(
    *,
    ...
    is what ``horo gateway start`` (no ``-p``) targets. Without it,
    bare ``horo gateway start`` inside the container would land on
    ...
    # ``horo gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `horo profile create` (config.yaml
            # is not — that comes later via `horo setup`). Use it as the
            ...
            # via ``horo -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the horo args.
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``horo`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``horo <subcommand>``, so an
    explicit leading ``horo`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `horo <subcommand>`; peel an explicit hermes.
    if args and Path(args[0]).name == "horo":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``horo dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `horo -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit horo
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the horo user) won't hit EACCES. See
