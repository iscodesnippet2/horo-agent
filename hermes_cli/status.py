...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``horo config``'s
...
def show_status(args):
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: horo portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: horo model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: horo auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: horo auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: horo model)"
    ...
    print(color("  Run 'horo doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'horo setup' to configure", Colors.DIM))
    print()
