"""horo claw — OpenClaw migration commands.

Usage:
    horo claw migrate              # Preview then migrate (always shows preview first)
    horo claw migrate --dry-run    # Preview only, no changes
    horo claw migrate --yes        # Skip confirmation prompt
    horo claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    horo claw migrate --no-backup  # Skip pre-migration snapshot
    horo claw cleanup              # Archive leftover OpenClaw directories
    horo claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'horo gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: horo claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'horo claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: horo claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: horo claw migrate --yes")
            return
    ...
    # restorable with `horo import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: horo import {backup_archive.name}")
        except Exception as e:
            print()
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: horo claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: horo claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  horo claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  horo claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  horo config set OPENROUTER_API_KEY sk-or-v1-...")
