...
``hermes-cli`` to the non-existent ``horo``. ``resolve_toolset('horo')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       ...
       #38798 shape, where ``cli`` held ``horo`` instead of ``hermes-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `horo tools` to reconfigure."
        )
    return warnings
