...
支持任意模型——[Nous Portal](https://portal.nousresearch.com)、[OpenRouter](https://openrouter.ai)（200+ 模型）、[NVIDIA NIM](https://build.nvidia.com)（Nemotron）、[小米 MiMo](https://platform.xiaomimimo.com)、[z.ai/GLM](https://z.ai)、[Kimi/Moonshot](https://platform.moonshot.ai)、[MiniMax](https://www.minimax.io)、[Hugging Face](https://huggingface.co)、OpenAI，或自定义端点。使用 `horo model` 即可切换——无需改代码，无锁定。
...
```bash
horo              # 交互式 CLI — 开始对话
horo model        # 选择 LLM 提供商和模型
horo tools        # 配置启用的工具
horo config set   # 设置单个配置项
horo gateway      # 启动消息网关（Telegram、Discord 等）
horo setup        # 运行完整设置向导（一次性配置所有内容）
horo claw migrate # 从 OpenClaw 迁移（如果来自 OpenClaw）
horo update       # 更新到最新版本
horo doctor       # 诊断问题
```
...
它会通过 OAuth 登录、把 Nous 设为推理服务商，并启用 Tool Gateway。随时用 `horo portal info` 查看路由状态。完整说明见 [Tool Gateway 文档](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway)。
...
Hermes 有两种入口：用 `horo` 启动终端 UI，或运行网关从 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与之对话。进入对话后，许多斜杠命令在两种界面中通用。

| 操作 | CLI | 消息平台 |
|------|-----|----------|
| 开始对话 | `horo` | 运行 `horo gateway setup` + `horo gateway start`，然后给机器人发消息 |
...
**首次安装时：** 安装向导（`horo setup`）会自动检测 `~/.openclaw` 并在配置开始前提供迁移选项。

**安装后任意时间：**

```bash
horo claw migrate              # 交互式迁移（完整预设）
horo claw migrate --dry-run    # 预览将要迁移的内容
horo claw migrate --preset user-data   # 仅迁移用户数据，不含密钥
horo claw migrate --overwrite  # 覆盖已有冲突
```
...
使用 `horo claw migrate --help` 查看所有选项，或使用 `openclaw-migration` 技能进行交互式代理引导迁移（含干运行预览）。
...
`horo update`、托管 venv、lazy dependencies、gateway 和 docs tooling 使用的布局。
